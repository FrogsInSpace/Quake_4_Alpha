...
userCmdString_t	userCmdStrings[] = {
	...
	{ "_impulse100",	UB_IMPULSE100 },
	{ "_impulse101",	UB_IMPULSE101 },
	{ "_impulse102",	UB_IMPULSE102 },
	{ "_impulse103",	UB_IMPULSE103 },
	{ "_impulse104",	UB_IMPULSE104 },
	{ "_impulse105",	UB_IMPULSE105 },
	{ "_impulse106",	UB_IMPULSE106 },
	{ "_impulse107",	UB_IMPULSE107 },
	{ "_impulse108",	UB_IMPULSE108 },
	{ "_impulse109",	UB_IMPULSE109 },
	{ "_impulse110",	UB_IMPULSE110 },
	{ "_impulse111",	UB_IMPULSE111 },
	{ "_impulse112",	UB_IMPULSE112 },
	{ "_impulse113",	UB_IMPULSE113 },
	{ "_impulse114",	UB_IMPULSE114 },
	{ "_impulse115",	UB_IMPULSE115 },
	{ "_impulse116",	UB_IMPULSE116 },
	{ "_impulse117",	UB_IMPULSE117 },
	{ "_impulse118",	UB_IMPULSE118 },
	{ "_impulse119",	UB_IMPULSE119 },
	{ "_impulse120",	UB_IMPULSE120 },
	{ "_impulse121",	UB_IMPULSE121 },
	{ "_impulse122",	UB_IMPULSE122 },
	{ "_impulse123",	UB_IMPULSE123 },
	{ "_impulse124",	UB_IMPULSE124 },
	{ "_impulse125",	UB_IMPULSE125 },
	{ "_impulse126",	UB_IMPULSE126 },
	{ "_impulse127",	UB_IMPULSE127 },
...
public:
	...
	void			SetSlowJoystick( int slow );
	void			StuffImpulse( int impulse );
...
private:
	...
	static idCVar	in_joystickLeftStickMove;
	...
	static idCVar	m_maxInput;
};

#if defined( _WIN32 ) && defined( Q4_RECON_ENGINE_PRIVATE )
static_assert( sizeof( usercmd_t ) == 0x28, "usercmd_t ABI drift" );
static_assert( sizeof( idUsercmdGenLocal ) == 0xD74, "idUsercmdGenLocal ABI drift" );
#endif
...
idCVar idUsercmdGenLocal::in_joystickLeftStickMove( "in_joystickLeftStickMove", "1", CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_BOOL, "left or right joystick controls move" );
...
idCVar idUsercmdGenLocal::m_maxInput( "m_maxInput", "0", CVAR_SYSTEM, "caps the amount of mouse movement possible" );
...
void idUsercmdGenLocal::AdjustAngles( void ) {
	...
	if ( toggled_run.on ^ in_alwaysRun.GetBool() ) {
		speed = idMath::M_MS2SEC * common->GetUserCmdMSec() * in_angleSpeedKey.GetFloat();
	} else {
		speed = idMath::M_MS2SEC * common->GetUserCmdMSec();
	}
...
void idUsercmdGenLocal::MouseMove( void ) {
	...
	if ( m_maxInput.GetFloat() != 0.0f ) {
		if ( idMath::Fabs( mx ) > m_maxInput.GetFloat() ) {
			mx = mx / idMath::Fabs( mx ) * m_maxInput.GetFloat();
		}
		if ( idMath::Fabs( my ) > m_maxInput.GetFloat() ) {
			my = my / idMath::Fabs( my ) * m_maxInput.GetFloat();
		}
	}
...
void idUsercmdGenLocal::JoystickMove( void ) {
	const float lookScale = slowJoystick * 0.01f;
	viewangles[YAW] -= joystickAxis[AXIS_LEFT_HORIZONTAL] * m_yaw.GetFloat() * lookScale;
	viewangles[PITCH] += joystickAxis[AXIS_LEFT_VERTICAL] * m_pitch.GetFloat() * lookScale;
	cmd.rightmove += idMath::Ftoi( joystickAxis[AXIS_RIGHT_HORIZONTAL] * ( 1.0f / MAX_AXIS_RANGE ) * KEY_MOVESPEED );
	cmd.forwardmove -= idMath::Ftoi( joystickAxis[AXIS_RIGHT_VERTICAL] * ( 1.0f / MAX_AXIS_RANGE ) * KEY_MOVESPEED );
}
...
void idUsercmdGenLocal::InitCurrent( void ) {
	...
	cmd.flags = flags;
	cmd.impulse = impulse;
	// The matching retail executable initializes the run bit unconditionally.
	// CmdButtons recalculates it for non-inhibited commands.
	cmd.buttons |= BUTTON_RUN;
...
void idUsercmdGenLocal::Joystick( void ) {
	const int numEvents = Sys_PollJoystickInputEvents();
	for ( int i = 0; i < numEvents; i++ ) {
		int action;
		int value;
		...
		if ( action >= J_ACTION_BUTTON_LEFT_SHOULDER && action <= J_ACTION_BUTTON_RIGHT_TRIGGER ) {
			Key( K_JOY1 + action, value != 0 );
			continue;
		}

		switch ( action ) {
			case J_DELTA_LEFT_HORIZONTAL:
				joystickAxis[in_joystickLeftStickMove.GetBool() ? AXIS_RIGHT_HORIZONTAL : AXIS_LEFT_HORIZONTAL] = value;
				break;
			case J_DELTA_LEFT_VERTICAL:
				joystickAxis[in_joystickLeftStickMove.GetBool() ? AXIS_RIGHT_VERTICAL : AXIS_LEFT_VERTICAL] = value;
				break;
			case J_DELTA_RIGHT_HORIZONTAL:
				joystickAxis[in_joystickLeftStickMove.GetBool() ? AXIS_LEFT_HORIZONTAL : AXIS_RIGHT_HORIZONTAL] = value;
				break;
			case J_DELTA_RIGHT_VERTICAL:
				joystickAxis[in_joystickLeftStickMove.GetBool() ? AXIS_LEFT_VERTICAL : AXIS_RIGHT_VERTICAL] = value;
				break;
			default:
				break;
		}
...
void idUsercmdGenLocal::SetSlowJoystick( int slow ) {
	slowJoystick = slow;
}

void idUsercmdGenLocal::StuffImpulse( int impulseValue ) {
	stuffedImpulse = impulseValue;
}
