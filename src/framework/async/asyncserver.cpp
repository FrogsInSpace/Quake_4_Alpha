...
netadr_t idAsyncServer::GetBoundAdr( void ) const {
	...
	address.ip[0] = 127;
	address.ip[3] = 1;
	address.port = serverPort.GetPort();
	return address;
}
...
/*
==================
idAsyncServer::GetClientAddress
idAsyncServer::GetClientGUID
idAsyncServer::GetTrafficStats
==================
*/
netadr_t idAsyncServer::GetClientAddress( int clientNum ) const {
	netadr_t badAddress;
	...
	return clients[clientNum].channel.GetRemoteAddress();
}

const char *idAsyncServer::GetClientGUID( int clientNum ) const {
	if ( clientNum < 0 || clientNum >= MAX_ASYNC_CLIENTS ) {
		return NULL;
	}
	return clients[clientNum].guid;
}

void idAsyncServer::GetTrafficStats( int &bytesSent, int &packetsSent, int &bytesReceived, int &packetsReceived ) const {
	serverPort.GetTrafficStats( bytesSent, packetsSent, bytesReceived, packetsReceived );
}
...
bool idAsyncServer::SendSnapshotToClient( int clientNum ) {
	...
	game->ServerWriteSnapshot( clientNum, client.snapshotSequence, msg, clientInPVS, MAX_ASYNC_CLIENTS, gameFrame );
...
void idAsyncServer::PrintLocalServerInfo( void ) {
	...
	common->Printf( "server '%s' IP = %s\nprotocol %d.%d OS mask 0x%x\n", 
					...
					Sys_NetAdrToString( GetBoundAdr() ),
...
void idAsyncServer::RunFrame( void ) {
	const int userCmdMSec = common->GetUserCmdMSec();
	int			i, msec, size;
	bool		newPacket;
	idBitMsg	msg;
	byte		msgBuf[MAX_MESSAGE_SIZE];
	...
	do {
		...
		do {
			...
			newPacket = serverPort.GetPacketBlocking( from, msgBuf, size, sizeof( msgBuf ), userCmdMSec - gameTimeResidual - 1 );
			if ( newPacket ) {
				msg.Init( msgBuf, sizeof( msgBuf ) );
				msg.SetSize( size );
				msg.BeginReading();
	...
	} while( gameTimeResidual < userCmdMSec );
	...
	while( gameTimeResidual >= userCmdMSec ) {
		...
		// advance game
		const bool lastCatchupFrame = gameTimeResidual < userCmdMSec * 2;
		gameReturn_t ret = game->RunFrame( userCmds[gameFrame & ( MAX_USERCMD_BACKUP - 1 ) ], com_editors, lastCatchupFrame, gameFrame );

		idAsyncNetwork::ExecuteSessionCommand( ret.sessionCommand );

		// update time
		gameFrame++;
		gameTime += userCmdMSec;
		gameTimeResidual -= userCmdMSec;
	}
