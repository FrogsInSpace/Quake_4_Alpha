...
/*
==================
idAsyncClient::GetServerAddress
idAsyncClient::GetTrafficStats
==================
*/
netadr_t idAsyncClient::GetServerAddress( void ) const {
	return serverAddress;
}

void idAsyncClient::GetTrafficStats( int &bytesSent, int &packetsSent, int &bytesReceived, int &packetsReceived ) const {
	clientPort.GetTrafficStats( bytesSent, packetsSent, bytesReceived, packetsReceived );
}
...
void idAsyncClient::SetupConnection( void ) {
	...
	if ( clientState == CS_CHALLENGING ) {
		...
		msg.WriteString( cvarSystem->GetCVarString( "password" ) );
		...
		if ( idAsyncNetwork::LANServer.GetBool() ) {
			...
			msg.WriteString( session->GetCDKey() );
...
void idAsyncClient::RunFrame( void ) {
	...
	do {
		...
		do {
			...
			newPacket = clientPort.GetPacketBlocking( from, msgBuf, size, sizeof( msgBuf ), userCmdMSec - ( gameTimeResidual + clientPredictTime ) - 1 );
			if ( newPacket ) {
				msg.Init( msgBuf, sizeof( msgBuf ) );
				msg.SetSize( size );
				msg.BeginReading();
	...
	} while( gameTimeResidual + clientPredictTime < userCmdMSec );
	...
	if ( clientState == CS_DISCONNECTED ) {
		usercmdGen->GetDirectUsercmd();
		gameTimeResidual = userCmdMSec - 1;
		clientPredictTime = 0;
		return;
	}
	...
	if ( gameTimeResidual + clientPredictTime >= userCmdMSec ) {
		lastFrameDelta = 0;
	}
	...
	while ( gameTimeResidual + clientPredictTime >= userCmdMSec ) {
		...
		while ( snapshotGameFrame < gameFrame ) {
			...
			bool lastPredictFrame = ( snapshotGameFrame + 1 >= gameFrame && gameTimeResidual + clientPredictTime < userCmdMSec );
