...
idUserInterface *idSessionLocal::GetActiveGUI( void ) {
	return guiActive;
}

int idSessionLocal::GetDesiredSoundWorld( bool on, bool &newSoundWorld ) {
	newSoundWorld = false;
	if ( !on ) {
		return SOUNDWORLD_NONE;
	}
	if ( common->IsToolActive() ) {
		return SOUNDWORLD_EDITOR;
	}
	if ( guiActive == guiMainMenu ) {
		newSoundWorld = true;
		return SOUNDWORLD_MENU;
	}
	return SOUNDWORLD_GAME;
}

const char *idSessionLocal::KeysFromBinding( const char *bind ) {
...
==============
*/
void idSessionLocal::StartMenu( const char *menuName ) {
	if ( guiActive == guiMainMenu ) {
		return;
	}
	...
	guiMainMenu->HandleNamedEvent( menuName != NULL ? menuName : "Init" );
	guiMainMenu->SetStateString( "game_list", common->GetLocalizedString( "#str_107212" ) );
...
void idSessionLocal::SetMainMenuGuiVars( void ) {
	...
	guiMsg->SetStateString( "visible_hasxp", "0" );
	...
	for( icmd = 0; icmd < args.Argc(); ) {
		...
		if ( !idStr::Icmp( cmd, "play" ) ) {
			if ( args.Argc() - icmd >= 1 ) {
				...
				soundSystem->PlayShaderDirectly( sw, snd );
			}
			continue;
		}
	}
	...
	for( i = 0; i < args.Argc(); ) {
		...
		if ( !idStr::Icmp( cmd, "play" ) ) {
			if ( args.Argc() - i >= 1 ) {
				...
				soundSystem->PlayShaderDirectly( menuSoundWorld, snd );
			}
			continue;
		}
	}
	...
	for( icmd = 0; icmd < args.Argc(); ) {
		...
		if ( !idStr::Icmp( cmd, "play" ) ) {
			if ( args.Argc() - icmd >= 1 ) {
				...
				soundSystem->PlayShaderDirectly( menuSoundWorld, snd, channel );
		...
		if ( !idStr::Icmp( cmd, "music" ) ) {
			if ( args.Argc() - icmd >= 1 ) {
				idStr snd = args.Argv( icmd++ );
				soundSystem->PlayShaderDirectly( menuSoundWorld, snd, 2 );
			}
			continue;
		}
...
void idSessionLocal::HandleNoteCommands( const char *menuCommand ) {
	...
	if ( idStr::Icmp( menuCommand,  "note" ) == 0 && mapSpawned ) {
		...
		for ( i = 0; i < fileList.Num(); i++ ) {
			...
			R_ScreenshotFilename( workNote, workName, ".tga", shotName );
