...
/*
 ...
 PDB owner: framework/file.cpp.  The retail routine treats each element as a
 four-byte integer unless the TT_FLOAT bit is set and emits one text row per
 structure.
 =================
 */
void idFile_Common::WriteNumericStructArray( int numStructElements, int tokenSubTypeStructElements[], int arrayCount, byte *arrayStorage, const char *prepend ) {
	byte *cursor = arrayStorage;
	for ( int arrayIndex = 0; arrayIndex < arrayCount; arrayIndex++ ) {
		WriteFloatString( "%s", prepend );
		for ( int elementIndex = 0; elementIndex < numStructElements; elementIndex++, cursor += sizeof( int ) ) {
			if ( tokenSubTypeStructElements[elementIndex] & TT_FLOAT ) {
				WriteFloatString( "%f ", *reinterpret_cast<float *>( cursor ) );
			} else {
				WriteFloatString( "%u ", *reinterpret_cast<unsigned int *>( cursor ) );
			}
		}
		WriteFloatString( "\n" );
	}
...
/*
 =================
 idFile_Common::WriteSyncId / ReadSyncId
 =================
 */
static const int FILE_SYNC_ID = 518508557;
