...
static_assert( sizeof( idDeclAF ) == 0xB0, "retail idDeclAF ABI drift" );
static_assert( sizeof( idDeclAF_Body ) == 0x260, "retail idDeclAF_Body ABI drift" );
static_assert( sizeof( idDeclAF_Constraint ) == 0x288, "retail idDeclAF_Constraint ABI drift" );
#endif
...
/*
================
idDeclAF_Constraint::idDeclAF_Constraint
================
*/
idDeclAF_Constraint::idDeclAF_Constraint() {
	type = DECLAF_CONSTRAINT_INVALID;
	friction = 0.0f;
	stretch = 0.0f;
	compress = 0.0f;
	damping = 0.0f;
...
size_t idDeclAF::Size( void ) const {
	return sizeof( idDeclAF ) + model.Allocated() + skin.Allocated() +
		bodies.Allocated() + constraints.Allocated();
}
...
================
*/
bool idDeclAF::Validate( const char *text, int textLength, idStr &report ) const {
