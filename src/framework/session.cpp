...
void idSessionLocal::Clear() {
	...
	rw = NULL;
	sw = SOUNDWORLD_GAME;
	menuSoundWorld = SOUNDWORLD_MENU;
	readDemo = NULL;
	writeDemo = NULL;
	renderdemoVersion = 0;
	cmdDemoFile = NULL;
...
void idSessionLocal::Shutdown() {
	...
	soundSystem->StopAllSounds( sw );
	soundSystem->StopAllSounds( menuSoundWorld );
...
void idSessionLocal::QuickMapChange( const char *mapName ) {
	MoveToNewMap( mapName );
}

bool idSessionLocal::GetMapSpawned( void ) {
	return mapSpawned;
}

bool idSessionLocal::GetLoadingSaveGame( void ) {
	return loadingSaveGame;
}

idRenderWorld *idSessionLocal::GetGameRenderWorld( void ) {
	return rw;
}
...
void idSessionLocal::StartWipe( const char *_wipeMaterial, bool hold ) {
	...
	wipeStopTic = wipeStartTic + 1000.0f / common->GetUserCmdMSec() * com_wipeSeconds.GetFloat();
	wipeHold = hold;
}
...
void idSessionLocal::ShowLoadingGui() {
	...
	while ( Sys_Milliseconds() < stop || force-- > 0 ) {
		com_frameTime = com_ticNumber * common->GetUserCmdMSec();
		session->Frame();
		session->UpdateScreen( false );
	}
#else
	int stop = com_ticNumber + 1000.0f / common->GetUserCmdMSec() * 1.0f;
...
void idSessionLocal::StopRecordingRenderDemo() {
	...
	soundSystem->StopWritingDemo( sw );
	rw->StopWritingDemo();
...
void idSessionLocal::StartPlayingRenderDemo( idStr demoName ) {
	...
	soundSystem->StopAllSounds( sw );
	soundSystem->PlayShaderDirectly( sw, "", 0 );
	soundSystem->StopAllSounds( menuSoundWorld );
	soundSystem->PlayShaderDirectly( menuSoundWorld, "", 0 );
...
void idSessionLocal::BeginAVICapture( const char *demoName ) {
	...
	aviDemoFrameCount = 0;
	aviTicStart = 0;
	soundSystem->AVIOpen( sw, va( "demos/%s/", aviDemoShortName.c_str() ), aviDemoShortName.c_str() );
}
...
void idSessionLocal::ExecuteMapChange( bool noFadeWipe ) {
	...
	if ( !reloadingSameMap ) {
		...
		soundSystem->BeginLevelLoad( mapString.c_str() );
	}
	...
	for ( i = 0; i < numClients; i++ ) {
		game->SetUserInfo( i, mapSpawnData.userInfo[i], idAsyncNetwork::client.IsActive() );
	...
	if ( !idAsyncNetwork::IsActive() && !loadingSaveGame ) {
		...
		for ( i = 0; i < 10; i++ ) {
			game->RunFrame( mapSpawnData.mapSpawnUsercmd, com_editors, true, lastGameTic );
		}
	}
...
void idSessionLocal::AdvanceRenderDemo( bool singleFrameOnly ) {
	...
	while( skipFrames > -1 ) {
		...
		if ( ds == DS_SOUND ) {
			soundSystem->ProcessDemoCommand( sw, readDemo );
			continue;
		}
...
void idSessionLocal::Frame() {
	...
	if ( aviCaptureMode ) {
		...
		float ratio = 30.0f / ( 1000.0f / common->GetUserCmdMSec() / com_aviDemoTics.GetInteger() );
	...
	if ( timeHitch ) {
		int	skip = timeHitch / common->GetUserCmdMSec();
		lastGameTic += skip;
		numCmdsToRun -= skip;
		timeHitch = 0;
	}
...
void idSessionLocal::RunGameTic() {
	...
	int	start = Sys_Milliseconds();
	gameReturn_t	ret = game->RunFrame( &cmd, com_editors, common->IsRenderableGameFrame(), lastGameTic );
...
void idSessionLocal::ClearCDKey( bool valid ) {
	bool validity[2] = { valid, true };
	ClearCDKey( validity );
}
...
const char *idSessionLocal::GetCDKey( void ) {
	return GetCDKey( false );
}
...
bool idSessionLocal::CheckKey( const char *key, bool netConnect, bool offlineValid ) {
	bool valid[2] = { offlineValid, true };
	return CheckKey( key, netConnect, valid );
}
...
===============
*/
void idSessionLocal::GetCurrentMapName( idStr &mapName ) {
	mapName = currentMapName;
}

void idSessionLocal::EmitClientAuth( void ) {
	EmitGameAuth();
}

void idSessionLocal::UpdateFriends( void ) {
}

void idSessionLocal::KillLoadingGUI( void ) {
	if ( guiActive == guiLoading ) {
		SetGUI( NULL, NULL );
	}
}

void idSessionLocal::PrecacheMPSettingsMenu( void ) {
	SetMainMenuGuiVars();
}

void idSessionLocal::SetLoadingText( const char *text ) {
	if ( guiLoading != NULL ) {
		guiLoading->SetStateString( "loadingText", text != NULL ? text : "" );
	}
}

void idSessionLocal::AddLoadingIcon( const char *icon ) {
	if ( guiLoading != NULL && icon != NULL ) {
		guiLoading->SetStateString( "loadingIcon", icon );
	}
}
