...
static pureExclusion_t pureExclusions[] = {
	...
	{ 0,	0,	"gfx/2d/bigchars.tga",						NULL,		excludeFullName },
	{ 0,	0,	"dds/gfx/2d/bigchars.dds",					NULL,		excludeFullName },
...
public:
...
#ifndef Q4_RECON_ENGINE_PRIVATE
	virtual fsPureReply_t	SetPureServerChecksums( const int pureChecksums[ MAX_PURE_PAKS ], int gamePakChecksum[ MAX_GAMEPAK_PER_OS ], int missingChecksums[ MAX_PURE_PAKS ], int *missingGamePakChecksum, int *restartGamePakChecksum );
#endif
	...
	virtual int				ReadFile( const char *relativePath, void **buffer, unsigned *timestamp );
	...
	virtual bool			AddExplicitPak( const char *path );
	virtual void			RemoveExplicitPak( const char *path );
	virtual bool			IsPakLoaded( const char *path );
...
static_assert( sizeof( idFileSystemLocal ) == 0x774, "retail idFileSystemLocal ABI drift" );
#endif
...
idCVar	idFileSystemLocal::fs_importpath( "fs_importpath", "", CVAR_SYSTEM | CVAR_INIT, "import path for development assets" );
#ifdef WIN32
...
The retail implementation deliberately bypasses the virtual-file-system
search paths: the caller supplied a complete operating-system path.
================
*/
...
void idFileSystemLocal::SetAssetLogName( const char *logName ) {
	...
	currentAssetLog += logName ? logName : "";
...
void idFileSystemLocal::WriteAssetLog( void ) {
	idStr filename = currentAssetLog;
	filename.SetFileExtension( "txt" );

	idFile *file = OpenFileWrite( filename.c_str(), "fs_savepath", false );
	if ( !file ) {
		common->Warning( "Couldn't write %s.\n", filename.c_str() );
		return;
	}

	for ( int i = 0; i < assetLog.Num(); i++ ) {
		file->Write( assetLog[i].c_str(), assetLog[i].Length() );
		file->Write( "\n", 1 );
	}
	file->Flush();
	CloseFile( file );
...
Quake 4 exposes the two internal Doom 3 copy helpers through the public
filesystem interface for the editor/import pipeline.
=================
*/
void idFileSystemLocal::CopyOSFile( const char *fromOSPath, const char *toOSPath ) {
	...
	common->Printf( "copy %s to %s\n", fromOSPath, toOSPath );
	source = OpenOSFile( fromOSPath, "rb" );
	if ( !source ) {
		return;
	}

	len = DirectFileLength( source );
	buffer = (byte *)Mem_Alloc( len );
	if ( fread( buffer, 1, len, source ) != (unsigned int)len ) {
		common->FatalError( "short read in idFileSystemLocal::CopyOSFile()\n" );
	}
	fclose( source );

	CreateOSPath( toOSPath );
#ifdef WIN32
	// The original import path could overwrite checked-in, read-only game data.
	if ( strstr( toOSPath, "/game/" ) ) {
		DWORD attributes = GetFileAttributesA( toOSPath );
		if ( attributes != INVALID_FILE_ATTRIBUTES && ( attributes & FILE_ATTRIBUTE_READONLY ) ) {
			SetFileAttributesA( toOSPath, attributes & ~FILE_ATTRIBUTE_READONLY );
		}
	}
#endif
	destination = OpenOSFile( toOSPath, "wb" );
	if ( !destination ) {
		common->Printf( "could not create destination file\n" );
		Mem_Free( buffer );
		return;
	}
	if ( fwrite( buffer, 1, len, destination ) != (unsigned int)len ) {
		common->FatalError( "Quake 4 cannot run due to insufficient disk space.\n" );
	}
	fclose( destination );
	Mem_Free( buffer );
}
...
void idFileSystemLocal::RemoveFile( const char *relativePath, const char *basePath ) {
	...
	OSPath = BuildOSPath( root, gameFolder, relativePath );
	remove( OSPath );
...
============
*/
int idFileSystemLocal::ReadFile( const char *relativePath, void **buffer, unsigned *timestamp ) {
	idFile *	f;
	byte *		buf;
	int			len;
	bool		isConfig;
...
/*
===============
idFileSystemLocal::AddExplicitPak
idFileSystemLocal::RemoveExplicitPak
idFileSystemLocal::IsPakLoaded

Retail Quake 4 exposes these on Win32 as well as Xenon.  Unlike AddZipFile,
the supplied path is already absolute and the pack is immediately searchable.
===============
*/
bool idFileSystemLocal::IsPakLoaded( const char *path ) {
	for ( searchpath_t *search = searchPaths; search; search = search->next ) {
		if ( search->pack && !search->pack->pakFilename.Cmp( path ) ) {
			return true;
		}
	}
	return false;
...
bool idFileSystemLocal::AddExplicitPak( const char *path ) {
	if ( IsPakLoaded( path ) ) {
		return true;
	}

	pack_t *pak = LoadZipFile( path );
	if ( !pak ) {
		common->Warning( "AddZipFile %s failed\n", path );
		return false;
	}
...
void idFileSystemLocal::RemoveExplicitPak( const char *path ) {
	...
	unzClose( pak->handle );
	delete [] pak->buildBuffer;
	if ( pak->addon_info ) {
		pak->addon_info->mapDecls.DeleteContents( true );
		delete pak->addon_info;
	}
	delete pak;
...
#ifndef Q4_RECON_ENGINE_PRIVATE
fsPureReply_t idFileSystemLocal::SetPureServerChecksums( const int pureChecksums[ MAX_PURE_PAKS ], int gamePakChecksums[ MAX_GAMEPAK_PER_OS ], int missingChecksums[ MAX_PURE_PAKS ], int *missingGamePakChecksum, int *restartGamePakChecksumOut ) {
	if ( restartGamePakChecksumOut != NULL ) {
		*restartGamePakChecksumOut = restartGamePakChecksum;
	}
	return SetPureServerChecksums( pureChecksums, gamePakChecksums ? gamePakChecksums[0] : 0, missingChecksums, missingGamePakChecksum );
}
#endif
...
findFile_t idFileSystemLocal::FindFile( const char *path ) {
	const bool scheduleAddons = true;
	pack_t *pak;
...
/*
===============
idFileSystemLocal::OSpathToImportPath
===============
*/
bool idFileSystemLocal::OSpathToImportPath( const char *osPath, idStr &iPath, bool stripTemp ) {
	const char *importPath = fs_importpath.GetString();
	const int importPathLength = idStr::Length( importPath );

	if ( idStr::Icmpn( osPath, importPath, importPathLength ) ) {
		iPath = osPath;
		return false;
	}

	const char *relative = osPath + importPathLength;
	if ( relative[0] == '/' || relative[0] == '\\' ) {
		relative++;
	}
	if ( stripTemp ) {
		if ( !idStr::Icmpn( relative, "work", 4 ) ) {
			relative += 4;
		}
		if ( !idStr::Icmpn( relative, "final", 5 ) ) {
			relative += 5;
		}
		if ( relative[0] == '/' || relative[0] == '\\' ) {
			relative++;
...
/*
===============
idFileSystemLocal::OpenImportFileRead
===============
*/
idFile *idFileSystemLocal::OpenImportFileRead( const char *filename ) {
	...
	if ( fs_debug.GetInteger() ) {
		common->Printf( "idFileSystemLocal::OpenImportFileRead: %s\n", filename );
	}

	idStr finalName;
	finalName.AppendPath( fs_importpath.GetString() );
	finalName.AppendPath( filename );
	common->DPrintf( "reading from: %s\n", finalName.c_str() );

	idFile_Permanent *file = new idFile_Permanent;
	file->o = OpenOSFile( finalName.c_str(), "rb" );
	if ( !file->o ) {
		delete file;
		return NULL;
	}
...
/*
===============
idFileSystemLocal::WriteDemoHeader
===============
*/
void idFileSystemLocal::WriteDemoHeader( idFile *file ) {
	file->WriteString( fs_game_base.GetString() );
	file->WriteString( fs_game.GetString() );
	file->WriteInt( serverPaks.Num() );
	...
	for ( int i = 0; i < serverPaks.Num(); i++ ) {
		file->WriteString( serverPaks[i]->pakFilename.c_str() );
		file->WriteInt( serverPaks[i]->checksum );
	}

	UpdateGamePakChecksums();
	// The original format records the producer OS before the six-entry table.
	file->WriteInt( BUILD_OS_ID );
	for ( int os = 0; os < MAX_GAME_OS; os++ ) {
		pack_t *gamePak = NULL;
		if ( gamePakForOS[os] ) {
			for ( searchpath_t *search = searchPaths; search; search = search->next ) {
				if ( search->pack && search->pack->checksum == gamePakForOS[os] ) {
					gamePak = search->pack;
					break;
				}
			}
		}
		file->WriteString( gamePak ? gamePak->pakFilename.c_str() : "" );
		file->WriteInt( gamePak ? gamePak->checksum : 0 );
	}
}

/*
===============
idFileSystemLocal::ReadDemoHeader

Return values are part of the demo playback protocol: -1 requests a
filesystem restart, 0 rejects playback, and 1 continues.
===============
*/
int idFileSystemLocal::ReadDemoHeader( idFile *file ) {
	idStr recordedGameBase;
	idStr recordedGame;
	idStr pakName;
	idStr gamePakNames[MAX_GAME_OS];
	int pakChecksums[MAX_PURE_PAKS];
	int missingChecksums[MAX_PURE_PAKS];
	int gamePakChecksums[MAX_GAME_OS];
	int recordedOS = 0;
	int numPaks = 0;
	int storedPaks = 0;

	memset( pakChecksums, 0, sizeof( pakChecksums ) );
	memset( missingChecksums, 0, sizeof( missingChecksums ) );
	memset( gamePakChecksums, 0, sizeof( gamePakChecksums ) );

	file->ReadString( recordedGameBase );
	file->ReadString( recordedGame );
	common->DPrintf( "fs_game_base: '%s'\n", recordedGameBase.c_str() );
	common->DPrintf( "fs_game: '%s'\n", recordedGame.c_str() );
	file->ReadInt( numPaks );

	if ( numPaks > 0 ) {
		for ( int i = 0; i < numPaks; i++ ) {
			int checksum;
			file->ReadString( pakName );
			file->ReadInt( checksum );
			common->DPrintf( "pak '%s' 0x%x\n", pakName.c_str(), checksum );
			if ( storedPaks < MAX_PURE_PAKS - 1 ) {
				pakChecksums[storedPaks++] = checksum;
			}
		}
		pakChecksums[storedPaks] = 0;

		file->ReadInt( recordedOS );
		common->DPrintf( "record on an OS id: %d, local playback on OS id %d\n", recordedOS, BUILD_OS_ID );
		for ( int os = 0; os < MAX_GAME_OS; os++ ) {
			file->ReadString( gamePakNames[os] );
			file->ReadInt( gamePakChecksums[os] );
			if ( gamePakChecksums[os] ) {
				common->DPrintf( "game pak OS %d: '%s' 0x%x\n", os, gamePakNames[os].c_str(), gamePakChecksums[os] );
			}
		}
	} else {
		common->DPrintf( "demo wasn't recorded in pure mode\n" );
	}

	const bool enforce = cvarSystem->GetCVarBool( "demo_enforceFS" );
	if ( recordedGameBase.Icmp( fs_game_base.GetString() ) || recordedGame.Icmp( fs_game.GetString() ) ) {
		common->Warning( "fs_game_base '%s' or fs_game '%s' are not matching the current configuration ( '%s' '%s' )",
			recordedGameBase.c_str(), recordedGame.c_str(), fs_game_base.GetString(), fs_game.GetString() );
		if ( enforce ) {
			common->Printf( "demo_enforceFS is enabled, need to reload engine before demo playback\n" );
			fs_game_base.SetString( recordedGameBase.c_str() );
			fs_game.SetString( recordedGame.c_str() );
			if ( numPaks > 0 ) {
				int gameChecksum = gamePakChecksums[BUILD_OS_ID];
				if ( !gameChecksum ) {
					common->Warning( "no game pak for this OS was present at recording" );
					gameChecksum = gameDLLChecksum;
				}
				SetRestartChecksums( pakChecksums, gameChecksum );
			}
			return -1;
		}
		common->Printf( "demo_enforceFS is off, demo might crash or generally not playback correctly\n" );
	}
	...
	const fsPureReply_t reply = SetPureServerChecksums( pakChecksums, gameChecksum, missingChecksums, &missingGamePakChecksum );
	...
	switch ( reply ) {
		...
		case PURE_RESTART:
			common->Warning( "a filesystem restart is needed to playback this demo" );
			if ( !enforce ) {
				common->Printf( "demo_enforceFS is off, demo might crash or generally not playback correctly\n" );
				return 1;
			}
		...
		case PURE_MISSING: {
			idStr checksums;
			int missingCount = 0;
			for ( ; missingCount < MAX_PURE_PAKS && missingChecksums[missingCount]; missingCount++ ) {
				checksums += va( "0x%x ", missingChecksums[missingCount] );
			}
			common->Warning( "missing %d pak files from initial demo recording: %s", missingCount, checksums.c_str() );
			if ( missingGamePakChecksum ) {
				common->Warning( "can't match the game code used to record this demo" );
			}
			if ( !enforce ) {
				common->Printf( "demo_enforceFS is off, demo might crash or generally not playback correctly\n" );
				return 1;
			}
			common->Printf( "demo_enforceFS is enabled, aborting playback\n" );
			return 0;
		}
