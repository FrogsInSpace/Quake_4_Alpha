...
static_assert( sizeof( idCVarSystemLocal ) == 0x48, "idCVarSystemLocal ABI drift" );
#endif
...
void idCVarSystemLocal::Register( const idCVarHelp *cvarHelp ) {
	cvarHelps.Append( new idCVarHelp( *cvarHelp ) );
}

idCVarHelp *idCVarSystemLocal::GetHelps( cvarHelpCategory_t category ) {
	idCVarHelp *first = NULL;
	idCVarHelp *last = NULL;
	for ( int i = 0; i < cvarHelps.Num(); i++ ) {
		idCVarHelp *help = cvarHelps[i];
		if ( !( category & help->GetCategory() ) ) {
			continue;
		}
		if ( last ) {
			last->SetNext( help );
...
unsigned int idCVarSystemLocal::WriteFlaggedVariables( int flags, const char *setCmd, byte *buf, unsigned int bufSize ) const {
	...
	for ( int i = 0; i < cvars.Num(); i++ ) {
		...
		*write++ = 0;
		...
		const int cvarFlags = cvar->GetFlags();
		if ( cvarFlags & CVAR_BOOL ) {
			if ( write >= end ) {
				return write - buf;
			}
			*write++ = cvar->GetBool();
		} else if ( cvarFlags & CVAR_INTEGER ) {
			if ( end - write < sizeof( int ) ) {
				return write - buf;
			}
			const int value = cvar->GetInteger();
			memcpy( write, &value, sizeof( value ) );
			write += sizeof( value );
		} else if ( cvarFlags & CVAR_FLOAT ) {
			if ( end - write < sizeof( float ) ) {
				return write - buf;
			}
...
void idCVarSystemLocal::ApplyFlaggedVariables( byte *buf, unsigned int bufSize ) {
	...
	byte *end = buf + bufSize;
	while ( read < end && *read ) {
		const char *name = reinterpret_cast<const char *>( read );
		while ( read < end && *read ) {
			read++;
		}
		if ( read >= end ) {
		...
		if ( cvarFlags & CVAR_BOOL ) {
			...
			cvar->SetBool( ( *read++ & 1 ) != 0 );
		} else if ( cvarFlags & CVAR_INTEGER ) {
			if ( end - read < sizeof( int ) ) {
				break;
			}
			int value;
			memcpy( &value, read, sizeof( value ) );
			read += sizeof( value );
			...
			const char *value = reinterpret_cast<const char *>( read );
			while ( read < end && *read ) {
				read++;
			}
			if ( read >= end ) {
...
idStr idCVarSystemLocal::WriteFlaggedVariables( int flags ) const {
	...
	for ( int i = 0; i < cvars.Num(); i++ ) {
		...
		infoString += "\\";
		infoString += cvar->GetName();
		infoString += "\\";
		infoString += cvar->GetString();
	}
	return infoString;
}
