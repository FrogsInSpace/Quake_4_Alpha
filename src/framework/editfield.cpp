...
void idEditField::Draw( int x, int y, int width, bool showCursor, const idMaterial *shader ) {
	...
	if ( len <= drawLen ) {
		...
		// Quake 4 supports several two-byte escape classes, not just colors.
		const int escapeLength = idStr::IsEscape( buffer + prestep );
		if ( escapeLength ) {
			prestep += escapeLength;
		}
	}
	...
	for ( int i = 0; i < cursor; i++ ) {
		const int escapeLength = idStr::IsEscape( &str[i] );
		if ( escapeLength ) {
			i += escapeLength - 1;
			prestep += escapeLength;
		}
