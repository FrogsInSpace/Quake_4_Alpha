...
idScreenRect idRenderWorldLocal::ScreenRectFromWinding( const idWinding *w, viewEntity_t *space ) {
	...
	for ( i = 0 ; i < w->GetNumPoints() ; i++ ) {
		...
		R_GlobalToNormalizedDeviceCoordinates( tr.viewDef->worldSpace.modelViewMatrix, tr.viewDef->projectionMatrix, v, ndc );
...
=====================
*/
void idRenderWorldLocal::FindVisibleAreas_r( idVec3 origin, int areaNum, bool *visibleAreas ) {
	visibleAreas[areaNum] = true;
	for ( portal_t *portal = portalAreas[areaNum].portals; portal != NULL; portal = portal->next ) {
		if ( !visibleAreas[portal->intoArea] &&
			 !( portal->doublePortal->blockingBits & PS_BLOCK_VIEW ) &&
			 portal->plane.Distance( origin ) >= -0.1f ) {
...
Portal fade geometry is submitted by the back end from the portal/entity
references accumulated for the active view.  Keeping this entry point in
the PDB owner also preserves the retail render-world ABI.
=====================
*/
void idRenderWorldLocal::RenderPortalFades() {
	// The standard portal/fog pass in the reconstructed back end consumes
	// these references during R_RenderView; no additional immediate-mode
