...
void GL_PolygonOffsetState( bool enable, float factor, float units ) {
	if ( enable ) {
		if ( !backEnd.glState.polyOffset ||
			...
			backEnd.glState.polyOffset = true;
			backEnd.glState.polyOffsetFactor = factor;
			backEnd.glState.polyOffsetUnits = units;
		}
	} else if ( backEnd.glState.polyOffset ) {
		qglDisable( GL_POLYGON_OFFSET_FILL );
		backEnd.glState.polyOffset = false;
	}
}

void GL_PolygonOffset( const idMaterial *material, bool enable ) {
	if ( material->TestMaterialFlag( MF_POLYGONOFFSET ) ) {
		GL_PolygonOffsetState( enable, r_offsetFactor.GetFloat(),
			r_offsetUnits.GetFloat() * material->GetPolygonOffset() );
	}
}

void GL_VertexAttribState( unsigned int vertexAttribStateBits ) {
	unsigned int changed = r_useStateCaching.GetBool() ?
		( vertexAttribStateBits ^ backEnd.glState.vertexAttribStateBits ) : ~0u;
	if ( changed == 0 ) {
		return;
	}
	backEnd.glState.vertexAttribStateBits = vertexAttribStateBits;

	const unsigned int legacyBits[3] = { BIT( 0 ), BIT( 2 ), BIT( 1 ) };
	const unsigned int legacyStates[3] = { GL_VERTEX_ARRAY, GL_COLOR_ARRAY, GL_NORMAL_ARRAY };
	for ( int i = 0; i < 3; ++i ) {
		if ( ( changed & legacyBits[i] ) != 0 ) {
			if ( ( vertexAttribStateBits & legacyBits[i] ) != 0 ) { qglEnableClientState( legacyStates[i] ); }
			else { qglDisableClientState( legacyStates[i] ); }
			changed &= ~legacyBits[i];
		}
	}

	if ( ( changed & 0x7F8u ) != 0 ) {
		const int activeTmu = backEnd.glState.currenttmu;
		for ( int unit = 0; unit < 8; ++unit ) {
			const unsigned int legacyTexCoord = BIT( 3 + unit );
			if ( ( changed & legacyTexCoord ) == 0 ) { continue; }
			if ( unit != backEnd.glState.currenttmu ) {
				qglClientActiveTextureARB( GL_TEXTURE0_ARB + unit );
				backEnd.glState.currenttmu = unit;
			}
			if ( ( vertexAttribStateBits & legacyTexCoord ) != 0 ) {
				const unsigned int genericTexCoord = BIT( 24 + unit );
				if ( ( changed & genericTexCoord ) != 0 ) {
					qglDisableVertexAttribArrayARB( 8 + unit );
					changed &= ~genericTexCoord;
				}
				qglEnableClientState( GL_TEXTURE_COORD_ARRAY );
			} else {
				qglDisableClientState( GL_TEXTURE_COORD_ARRAY );
			}
			changed &= ~legacyTexCoord;
		}
		if ( activeTmu != backEnd.glState.currenttmu ) {
			qglClientActiveTextureARB( GL_TEXTURE0_ARB + activeTmu );
			backEnd.glState.currenttmu = activeTmu;
		}
	}

	const int genericAttribs[] = { 1, 5, 6, 7, 8, 9, 10, 11 };
	const unsigned int genericMasks[] = {
		BIT( 17 ), BIT( 21 ), BIT( 22 ), BIT( 23 ),
		BIT( 24 ), BIT( 25 ), BIT( 26 ), BIT( 27 )
	};
	for ( int i = 0; i < 8; ++i ) {
		if ( ( changed & genericMasks[i] ) == 0 ) { continue; }
		if ( ( vertexAttribStateBits & genericMasks[i] ) != 0 ) { qglEnableVertexAttribArrayARB( genericAttribs[i] ); }
		else { qglDisableVertexAttribArrayARB( genericAttribs[i] ); }
	}
}
...
void GL_DisableTexCoordState() {
	GL_VertexAttribState( backEnd.glState.vertexAttribStateBits & ~BIT( 3 + backEnd.glState.currenttmu ) );
}
...
void RB_SetDefaultGLState( void ) {
	...
	// Disable every tracked array before resetting the cached state.
	...
	GL_SelectTexture( 0 );
	GL_VertexAttribState( BIT( 0 ) | BIT( 3 ) );
	...
	for ( ; cmds ; cmds = (const emptyCommand_t *)cmds->next ) {
		...
		case RC_COPY_RENDER:
			if ( !r_skipCopyTexture.GetBool() ) {
				...
				if ( copy->image ) {
					copy->image->CopyFramebuffer( copy->x, copy->y,
						copy->imageWidth, copy->imageHeight, false );
				}
			}
			c_copyRenders++;
			break;
