...
void RB_PrepareStageTexturing( const shaderStage_t *pStage, const drawSurf_t *surf,
		idDrawVert *ac, bool fillingDepth ) {
	...
	if ( pStage->texture.texgen == TG_GLASSWARP && tr.backEndRenderer == BE_ARB2 ) {
		...
		float mat[16], plane[4];
		myGlMultMatrix( surf->space->modelViewMatrix, backEnd.viewDef->projectionMatrix, mat );
		plane[0] = mat[0]; plane[1] = mat[4]; plane[2] = mat[8]; plane[3] = mat[12];
		qglTexGenfv( GL_S, GL_OBJECT_PLANE, plane );
		plane[0] = mat[1]; plane[1] = mat[5]; plane[2] = mat[9]; plane[3] = mat[13];
		qglTexGenfv( GL_T, GL_OBJECT_PLANE, plane );
		plane[0] = mat[3]; plane[1] = mat[7]; plane[2] = mat[11]; plane[3] = mat[15];
		qglTexGenfv( GL_Q, GL_OBJECT_PLANE, plane );
		GL_SelectTexture( 0 );
	}
...
void RB_T_FillDepthBuffer( const drawSurf_t *surf ) {
	...
	if ( backEnd.viewDef->numClipPlanes && surf->space != backEnd.currentSpace ) {
		...
		GLdouble clipPlane[4] = { plane[0], plane[1], plane[2], plane[3] };
	...
	if ( shader->Coverage() == MC_PERFORATED ) {
		...
		bool didDraw = false;
		const bool useAlphaToCoverage = r_alphaToCoverage.GetBool() && r_multiSamples.GetInteger() > 0;
		...
		for ( stage = 0; stage < shader->GetNumStages() ; stage++ ) {		
			...
			if ( useAlphaToCoverage &&
				( pStage->alphaTestMode == GL_GREATER || pStage->alphaTestMode == GL_GEQUAL ) &&
				alphaRef >= 0.4f && alphaRef < 0.6f ) {
				if ( alphaTestEnabled ) {
				...
				qglAlphaFunc( pStage->alphaTestMode, alphaRef );
			}
			...
			RB_FinishStageTexturing( pStage, surf, ac, true );
		}
		if ( alphaTestEnabled ) {
			qglDisable( GL_ALPHA_TEST );
		} else {
...
void RB_STD_FillDepthBuffer( drawSurf_t **drawSurfs, int numDrawSurfs ) {
	...
	if ( backEnd.viewDef->numClipPlanes ) {
		...
		GL_DisableVertexAttribState( 0x10 );
		qglEnable( GL_TEXTURE_GEN_S );
		qglTexCoord2f( 1, 0.5 );
	}
...
void RB_GetCurrentRender( bool force ) {
	backEnd.currentRenderNeeded = false;
	if ( !r_skipPostProcess.GetBool() && glConfig.allowARB2Path &&
		( backEnd.viewDef->renderFlags & RF_PRIMARY_VIEW ) &&
		( force || !backEnd.currentRenderCopied ) && tr.backEndRenderer == BE_ARB2 ) {
...
void RB_STD_T_RenderShaderPasses( const drawSurf_t *surf ) {
	...
	if ( surf->space->weaponDepthHackInViewID &&
		surf->space->weaponDepthHackInViewID == backEnd.viewDef->renderView.viewID ) {
	...
	for ( stage = 0; stage < shader->GetNumStages() ; stage++ ) {		
		...
		if ( newStage ) {
			...
			if ( tri->primBatchMesh ) {
				if ( newStage->md5rVertexProgram > 0 ) {
					qglBindProgramARB( GL_VERTEX_PROGRAM_ARB, newStage->md5rVertexProgram );
					RB_ARB2_LoadLocalViewOriginIntoVPParams( surf );
					RB_ARB2_LoadMVPMatrixIntoVPParams( surf );
					RB_ARB2_LoadProjectionMatrixIntoVPParams( surf );
				...
				qglVertexAttribPointerARB( 10, 3, GL_FLOAT, false, sizeof( idDrawVert ), ac->tangents[1].ToFloatPtr() );
				qglNormalPointer( GL_FLOAT, sizeof( idDrawVert ), ac->normal.ToFloatPtr() );
				GL_VertexAttribState( 0x0600000F );
				qglBindProgramARB( GL_VERTEX_PROGRAM_ARB, newStage->vertexProgram );
			}
		...
		// bind the texture
		RB_BindVariableStageImage( &pStage->texture );
		...
		RB_FinishStageTexturing( pStage, surf, ac, false );
...
int RB_STD_DrawShaderPasses( drawSurf_t **drawSurfs, int numDrawSurfs,
		materialSort_t minSort, materialSort_t maxSort ) {
	int i;

	RB_LogComment( "---------- RB_STD_DrawShaderPasses ----------\n" );

	if ( minSort == SS_POST_PROCESS && !backEnd.viewDef->isSubview &&
		backEnd.currentRenderNeeded ) {
		RB_GetCurrentRender( false );
	}
	...
	for ( i = 0; i < numDrawSurfs; i++ ) {
		...
		if ( minSort != SS_POST_PROCESS ) {
			if ( material->TestMaterialFlag( MF_NEED_CURRENT_RENDER ) &&
				...
				RB_GetCurrentRender( true );
			}
			if ( !material->TestMaterialFlag( MF_NEED_CURRENT_RENDER ) ||
				backEnd.currentRenderCopied ) {
				RB_STD_T_RenderShaderPasses( drawSurfs[i] );
			}
		} else if ( backEnd.currentRenderCopied ) {
			RB_STD_T_RenderShaderPasses( drawSurfs[i] );
		}
...
static void RB_T_Shadow( const drawSurf_t *surf ) {
	...
	if ( tr.backEndRendererHasVertexPrograms && r_useShadowVertexProgram.GetBool()
		&& surf->space != backEnd.currentSpace && !surf->geo->primBatchMesh ) {
		idVec4 localLight;
...
rvVertexFormat supportedMD5RBasicFogVertexFormats[3] = {
...
static void RB_T_BasicFog( const drawSurf_t *surf ) {
	...
	if ( mesh ) {
		...
		for ( formatIndex = 0; formatIndex < 3; ++formatIndex ) {
			if ( drawFormat->HasSameComponents( supportedMD5RBasicFogVertexFormats[formatIndex] ) ) {
				break;
			}
		}
		if ( formatIndex < 3 ) {
			mesh->SetupForDrawRender( &supportedMD5RBasicFogVertexFormats[formatIndex] );
			qglBindProgramARB( GL_VERTEX_PROGRAM_ARB, 45 + formatIndex );
		}
		RB_ARB2_LoadMVPMatrixIntoVPParams( surf );
		...
		local[3] += 0.5f;
		qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 93, local.ToFloatPtr() );
		local[0] = local[1] = local[2] = 0.0f;
		local[3] = 0.5f;
		qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 94, local.ToFloatPtr() );
		R_GlobalPlaneToLocal( surf->space->modelMatrix, fogPlanes[2], local );
		local[3] += FOG_ENTER;
		qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 95, local.ToFloatPtr() );
		R_GlobalPlaneToLocal( surf->space->modelMatrix, fogPlanes[3], local );
		qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 96, local.ToFloatPtr() );
...
static void RB_FogPass( const drawSurf_t *drawSurfs,  const drawSurf_t *drawSurfs2 ) {
	...
	GL_DisableVertexAttribState( 16 );
	qglEnable( GL_TEXTURE_GEN_S );
	qglEnable( GL_TEXTURE_GEN_T );
