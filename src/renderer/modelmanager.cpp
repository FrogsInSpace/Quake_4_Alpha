...
public:
	...
	virtual srfTriangles_t *CreateShadowVolume( const srfTriangles_t *tri, const idRenderLight *light, int optimize );
	virtual idRenderLight *CreateLightDef();
	virtual void			FreeLightDef( idRenderLight *light );
	...
	virtual size_t			ListModelSummary();
...
void idRenderModelManagerLocal::Reset() {
...
void idRenderModelManagerLocal::CleanupTriangles( srfTriangles_t *tris, bool createNormals,
		bool identifySilEdges, bool useUnsmoothedTangents, bool needSilMultiply ) {
	(void)needSilMultiply;
...
idRenderLight *idRenderModelManagerLocal::CreateLightDef() {
	return new idRenderLightLocal;
}
...
bool idRenderModelManagerLocal::CheckModel( idRenderModel *model ) {
	...
	return models.FindIndex( model ) >= 0;
}

size_t idRenderModelManagerLocal::ListModelSummary() {
	size_t total = 0;
	for ( int i = 0; i < models.Num(); ++i ) {
		if ( models[i] && models[i]->IsLoaded() ) {
			total += models[i]->Memory();
		}
	}
	return total;
}

srfTriangles_t *idRenderModelManagerLocal::CreateShadowVolume( const srfTriangles_t *tri,
		const idRenderLight *light, int optimize ) {
	...
	srfTriangles_t *result = R_CreateShadowVolume( &entity, tri,
		static_cast<const idRenderLightLocal *>( light ), optimize, cullInfo );
...
=================
*/
void idRenderModelManagerLocal::PrintMemInfo( MemInfo *mi ) {
	int i, j, totalMem = 0;
	int *sortIndex;
	idFile *f;
