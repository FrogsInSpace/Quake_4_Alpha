...
idCVar image_writeProgramImages( "image_writeProgramImages", "0", CVAR_RENDERER | CVAR_BOOL, "copy program targas to compression directory", idCmdSystem::ArgCompletion_Boolean );
// do this with a pointer, in case we want to make the actual manager
// a private virtual subclass
idImageManager	imageManager;
idImageManager	*globalImages = &imageManager;

// Retail Image_init.obj supplies this empty base hook. Pbuffer-backed images
// override it to restore the previous render context.
void idImage::UnBind() {
}
...
idPBufferImage *idImageManager::AllocPBufferImage( const char *name ) {
	if ( strlen( name ) >= MAX_IMAGE_NAME ) {
		common->Error( "idImageManager::AllocPBufferImage: \"%s\" is too long\n", name );
	}

	idStr imageName = name;
	const int hash = imageName.FileNameHash();
	for ( idImage *existing = imageHashTable[hash]; existing != NULL; existing = existing->hashNext ) {
		if ( imageName.Icmp( existing->imgName ) == 0 ) {
			idPBufferImage *pbufferImage = dynamic_cast<idPBufferImage *>( existing );
			if ( pbufferImage == NULL ) {
	...
	idPBufferImage *image = new idPBufferImage;
	images.Append( image );
	image->hashNext = imageHashTable[hash];
	imageHashTable[hash] = image;
	image->imgName = name;
	qglGenTextures( 1, &image->texnum );
	image->mRenderTarget = new rvTexRenderTarget;
	return image;
}
...
void idImageManager::Init() {
	...
	reflectionImage = ImageFromFunction("_reflectionRender", R_RGBA8Image );
...
===============
*/
void idImageManager::PrintMemInfo( MemInfo *mi ) {
	int i, j, total = 0;
	int *sortIndex;
	idFile *f;
