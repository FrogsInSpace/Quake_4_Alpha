...
idVec4 showBatchSizeReallyBadColor( 1.0f, 0.0f, 1.0f, 1.0f );
idVec4 showBatchSizeBadColor( 1.0f, 0.0f, 0.0f, 1.0f );
idVec4 showBatchSizeModerateColor( 1.0f, 1.0f, 0.0f, 1.0f );
idVec4 showBatchSizeGoodColor( 0.0f, 1.0f, 0.0f, 1.0f );
...
void RB_DrawElementsWithCounters( const srfTriangles_t *tri ) {
	if ( tri->primBatchMesh ) {
		...
		if ( tri->indexes ) {
			tri->primBatchMesh->Draw( tri->skinToModelTransforms, tri->indexes,
				tri->numIndexes, NULL );
		} else {
			tri->primBatchMesh->Draw( tri->skinToModelTransforms, NULL );
		}
		return;
	}
...
void RB_T_ShowBatchSizeRenderTriangleSurface( const drawSurf_t *surf ) {
	idVec4 color;
	const int numIndexes = surf->geo->numIndexes;
	if ( numIndexes <= 30 ) {
		color = showBatchSizeReallyBadColor;
	} else if ( numIndexes <= 600 ) {
		color.Lerp( showBatchSizeBadColor, showBatchSizeModerateColor,
			(float)numIndexes / 600.0f );
	} else if ( numIndexes <= 1500 ) {
		color.Lerp( showBatchSizeModerateColor, showBatchSizeGoodColor,
			(float)numIndexes / 1500.0f );
	} else {
		color = showBatchSizeGoodColor;
	}
...
void RB_T_ShowTriRenderTriangleSurface( const drawSurf_t *surf ) {
	if ( !( com_editors & EDITOR_MODVIEW ) ) {
		RB_RenderTriangleSurface( surf->geo );
		return;
	}
	if ( r_showTris.GetBool() || surf->geo->modviewSelected ) {
...
void RB_T_RenderSimpleSurface( const drawSurf_t *surf ) {
	...
	qglColor4f( 1.0f, 1.0f, 1.0f, surf->material->GetEditorAlpha() );
	surf->material->GetEditorImage()->Bind();
	RB_RenderTriangleSurface( surf->geo );
}

void RB_T_RenderTriangleSurfacePasses( const drawSurf_t *surf ) {
	int passes = surf->material ? surf->material->GetNumStages() : 0;
	if ( r_showOverDraw.GetInteger() > 1 && passes ) {
		passes = 1;
	}
...
void RB_EnterWeaponDepthHack() {
	qglDepthRange( 0, 0.5 );
	backEnd.projectionMatrix[14] *= 0.25f;

	qglMatrixMode(GL_PROJECTION);
	qglLoadMatrixf( backEnd.projectionMatrix );
	backEnd.mvpSpace = NULL;
...
void RB_EnterModelDepthHack( float depth ) {
	...
	backEnd.projectionMatrix[14] -= depth;
...
void RB_RenderDrawSurfListWithFunction( drawSurf_t **drawSurfs, int numDrawSurfs, 
	...
	for (i = 0  ; i < numDrawSurfs ; i++ ) {
		...
		if ( drawSurf->space->weaponDepthHackInViewID &&
			drawSurf->space->weaponDepthHackInViewID == backEnd.viewDef->renderView.viewID ) {
			RB_EnterWeaponDepthHack();
		}
...
static void RB_SubmittInteraction( drawInteraction_t *din, void (*DrawInteraction)(const drawInteraction_t *) ) {
	...
	if ( !din->diffuseImage || r_skipDiffuse.GetBool() || r_skipTextures.GetBool() ) {
		din->diffuseImage = globalImages->blackImage;
	}
	if ( !din->specularImage || r_skipSpecular.GetBool() || din->ambientLight || r_skipTextures.GetBool() ) {
		din->specularImage = globalImages->blackImage;
	}
	if ( !din->bumpImage || r_skipBump.GetBool() || r_skipTextures.GetBool() ) {
...
void RB_CreateSingleDrawInteractions( const drawSurf_t *surf, void (*DrawInteraction)(const drawInteraction_t *) ) {
	...
	if ( r_skipInteractions.GetBool() || !surf->geo ||
		( !surf->geo->ambientCache && !surf->geo->primBatchMesh ) ) {
		return;
	}
	...
	for ( int lightStageNum = 0 ; lightStageNum < lightShader->GetNumStages() ; lightStageNum++ ) {
		...
		if ( lightRegs && !lightRegs[ lightStage->conditionRegister ] ) {
			continue;
		}
...
void RB_DrawView( const void *data ) {
	...
	memcpy( backEnd.projectionMatrix, backEnd.viewDef->projectionMatrix,
