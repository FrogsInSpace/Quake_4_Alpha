...
typedef struct portalArea_s {
	...
	areaReference_t	effectRefs;		// Quake 4 BSE effects touching this area
	bool			hasSkybox;		// set while the area's world model is installed
} portalArea_t;
...
public:
	...
	virtual void			WriteRenderLight( idDemoFile *demo, const renderLight_t *light );
	virtual void			ReadRenderLight( idDemoFile *demo, renderLight_t &light );

	virtual qhandle_t		AddEffectDef( const renderEffect_t *effect, int time );
	virtual bool			UpdateEffectDef( qhandle_t effectHandle, const renderEffect_t *effect, int time );
	...
	virtual bool			EffectDefHasSound( const renderEffect_t *effect );
	virtual void			PushMarkedDefs();
	virtual void			ClearMarkedDefs();
	...
	virtual void			RenderPortalFades();
	virtual idVec3			WorldToScreen( renderView_t *renderView, idVec3 worldPoint );
	virtual idBounds		WorldToScreen( renderView_t *renderView, idBounds worldBounds );
	...
	virtual void			GetPortals( int areaNum, exitPortal_t *portals, int maxPortals );
	virtual void			GetPortal( int areaNum, int portalNum, exitPortal_t *portal );
	virtual bool			ProcessDemoCommand( idDemoFile *demo, renderView_t *renderView, renderView_t *portalRenderView, int *demoTimeOffset );
	...
	virtual void			ShowDebugLines();
	virtual void			ShowDebugPolygons();
	virtual void			ShowDebugText();
	virtual void			DebugFOV( const idVec4 &color, const idVec3 &origin, const idVec3 &dir, float fovX, float fovY, float radius, float nearDist, float farDist, int lifetime );
	...
	idList<int>					markedEntityDefs;
	idList<int>					markedLightDefs;
	idList<int>					markedEffectDefs;

	idBlockAlloc<areaReference_t, 1024, MA_RENDER> areaReferenceAllocator;
	idBlockAlloc<idInteraction, 256, MA_RENDER>	interactionAllocator;
	idBlockAlloc<areaNumRef_t, 1024, MA_RENDER>	areaNumRefAllocator;
	...
	void					FindVisibleAreas_r( idVec3 origin, int areaNum, bool *visibleAreas );
