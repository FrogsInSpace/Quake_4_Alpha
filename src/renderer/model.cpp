...
idRenderModel::~idRenderModel() {
	// Retail asserts this condition through the model manager.  Keep the check
	// non-fatal so shutdown can unwind cleanly while reconstructed subsystems
	// are being brought down.
	if ( renderModelManager && renderModelManager->CheckModel( this ) ) {
		common->Warning( "model freed while still registered" );
	}
}

bool idRenderModel::HasSeparateSilTraceMeshes() const {
	return false;
}
...
void idRenderModelStatic::InitEmptyFromArgs( const char *modelName, idDict &args ) {
	(void)args;
	InitEmpty( modelName );
}

int idRenderModelStatic::GetSurfaceMask( const char *surfaceName ) const {
	const idMaterial *material = declManager->FindMaterial( surfaceName, false );
	if ( !material ) {
		return 0;
	}
	int mask = 0;
	for ( int i = 0; i < surfaces.Num() && i < 32; ++i ) {
...
bool idRenderModelStatic::GetHasSky() const {
	return hasSky;
}

void idRenderModelStatic::SetViewEntity( const viewEntity_t *ve ) {
	(void)ve;
}

bool idRenderModelStatic::HasCollisionSurface( const renderEntity_t *ent ) const {
	for ( int i = 0; i < surfaces.Num(); ++i ) {
		const modelSurface_t &surface = surfaces[i];
		if ( surface.id == -1 || !surface.shader ) {
			continue;
		}
		const idMaterial *material = R_RemapShaderBySkin( surface.shader,
			ent ? ent->customSkin : NULL, ent ? ent->customShader : NULL );
		if ( material && ( material->GetSurfaceFlags() & SURF_COLLISION ) && !material->IsDrawn() ) {
			return true;
		}
	}
	return false;
