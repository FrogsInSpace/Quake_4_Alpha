...
void R_GlobalToNormalizedDeviceCoordinates( float *modelViewMatrix, float *projectionMatrix, const idVec3 &global, idVec3 &ndc ) {
	...
	for ( i = 0 ; i < 4 ; i ++ ) {
		view[i] = 
			global[0] * modelViewMatrix[ i + 0 * 4 ] +
			global[1] * modelViewMatrix[ i + 1 * 4 ] +
			global[2] * modelViewMatrix[ i + 2 * 4 ] +
			modelViewMatrix[ i + 3 * 4 ];
	}

	for ( i = 0 ; i < 4 ; i ++ ) {
		clip[i] = 
			view[0] * projectionMatrix[ i + 0 * 4 ] +
			view[1] * projectionMatrix[ i + 1 * 4 ] +
			view[2] * projectionMatrix[ i + 2 * 4 ] +
			view[3] * projectionMatrix[ i + 3 * 4 ];
	}
