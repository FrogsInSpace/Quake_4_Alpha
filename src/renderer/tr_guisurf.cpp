...
idScreenRect R_ScreenRectFromDrawSurf( drawSurf_t *drawSurf ) {
	...
	if ( tri->primBatchMesh ) {
		tri->primBatchMesh->ScreenRectFromDrawSurf( r,
	...
	for ( int i = 0; i < tri->numVerts; i++ ) {
		idVec3 global;
		idVec3 ndc;
		R_LocalPointToGlobal( drawSurf->space->modelMatrix, tri->verts[i].xyz, global );
		R_GlobalToNormalizedDeviceCoordinates( tr.viewDef->worldSpace.modelViewMatrix,
			tr.viewDef->projectionMatrix, global, ndc );

		const float y = tr.viewDef->viewport.y2 -
			( tr.viewDef->viewport.y2 - tr.viewDef->viewport.y1 ) * ( ndc.y + 1.0f ) * 0.5f;
		const float x = ( tr.viewDef->viewport.x2 - tr.viewDef->viewport.x1 ) *
			( ndc.x + 1.0f ) * 0.5f;
		r.AddPoint( x, y );
	}
...
void R_SurfaceToTextureAxis( const srfTriangles_t *tri, idVec3 &origin, idVec3 axis[3] ) {
	...
	if ( tri->primBatchMesh ) {
		tri->primBatchMesh->SurfaceToTextureAxis( origin, axis, tri->silTraceVerts );
		return;
	}
...
void R_RenderGuiSurf( idUserInterface *gui, drawSurf_t *drawSurf ) {
	...
	tr.guiModel->EmitToCurrentView( modelMatrix,
		drawSurf->space->weaponDepthHackInViewID,
		drawSurf->space->entityDef->parms.shaderParms );
	tr.guiModel->Clear();

	if ( atoi( gui->State().GetString( "2d_calc", "0" ) ) ) {
		const idScreenRect rect = R_ScreenRectFromDrawSurf( drawSurf );
		const float ratioX = 640.0f / ( tr.viewDef->viewport.x2 - tr.viewDef->viewport.x1 );
		const float ratioY = 480.0f / ( tr.viewDef->viewport.y2 - tr.viewDef->viewport.y1 );
