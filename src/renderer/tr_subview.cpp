...
static void R_PlaneForSurface( const srfTriangles_t *tri, idPlane &plane ) {
	if ( tri->primBatchMesh ) {
		tri->primBatchMesh->PlaneForSurface( plane, tri->silTraceVerts );
		return;
	}
...
bool R_PreciseCullSurface( const drawSurf_t *drawSurf, idBounds &ndcBounds ) {
	...
	for ( i = 0; i < tri->numVerts; i++ ) {
		...
		const idVec3 &point = tri->silTraceVerts
			? tri->silTraceVerts[i].xyzw.ToVec3()
			: tri->verts[i].xyz;

		R_TransformModelToClip( point, drawSurf->space->modelViewMatrix,
	...
	if ( tri->silTraceVerts ) {
		if ( tri->primBatchMesh->PreciseCullSurface( ndcBounds, tri->silTraceVerts,
				localView, drawSurf->space->modelMatrix ) ) {
			return true;
		}
	...
	for ( i = 0; i < tri->numIndexes; i += 3 ) {
		...
		for ( j = 0; j < w.GetNumPoints(); j++ ) {
			...
			R_GlobalToNormalizedDeviceCoordinates( tr.viewDef->worldSpace.modelViewMatrix, tr.viewDef->projectionMatrix, w[j].ToVec3(), screen );
...
static viewDef_t *R_MirrorViewBySurface( drawSurf_t *drawSurf ) {
	...
	*parms = *tr.viewDef;
	...
	viewOrigin += originalPlane.Normal();
...
static void R_RemoteRender( drawSurf_t *surf, textureStage_t *stage ) {
	...
	*parms = *tr.viewDef;
	...
	if ( idImageManager::image_downSize.GetInteger() ) {
		const int limit = idImageManager::image_downSizeLimit.GetInteger();
		if ( stage->height <= stage->width ) {
			tr.CropRenderSize( limit, limit, true );
		} else {
			tr.CropRenderSize( stage->width / ( stage->height / limit ), limit, true );
		}
	} else {
...
void R_ReflectionRender( drawSurf_t *surf, textureStage_t *stage, idScreenRect scissor ) {
	...
	stage->dynamicFrameCount = tr.frameCount;
	tr.CaptureRenderToImage( globalImages->reflectionImage->imgName );
	tr.UnCrop();
}
