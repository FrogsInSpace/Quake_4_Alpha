...
idRenderModel *	idRenderModelSprite::InstantiateDynamicModel( const struct renderEntity_s *renderEntity, const struct viewDef_s *viewDef, idRenderModel *cachedModel, dword surfMask ) {
	...
	if ( cachedModel != NULL ) {
		...
		// Keep the transient snapshot and its surface allocations in the same
		// renderer heap as the source model, as in Raven's NewRenderModel path.
		staticModel = NewRenderModel<idRenderModelStatic>( this );
