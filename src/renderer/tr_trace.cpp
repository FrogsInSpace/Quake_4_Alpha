...
Resolve a material-type image at the traced triangle's interpolated texture
coordinate.  Materials without a type image use their default type.
=================
*/
const rvDeclMatType *R_GetMaterialType( const idMaterial *shader,
	...
	const int firstIndex = triangleNum * 3;
	idVec3 vertices[3];
	idVec2 textureCoordinates[3];
	for ( int corner = 0; corner < 3; ++corner ) {
		const idDrawVert &vertex = tri->verts[tri->indexes[firstIndex + corner]];
		vertices[corner] = vertex.xyz;
		textureCoordinates[corner] = vertex.st;
	}

	const idVec3 normal = tri->facePlanes[triangleNum].Normal();
	const float area = idMath::BarycentricTriangleArea( normal,
	...
	idMath::BarycentricEvaluate( textureCoordinate, point, normal, area,
		vertices, textureCoordinates );
	return shader->GetMaterialType( textureCoordinate );
}
...
localTrace_t R_LocalTrace( const idVec3 &start, const idVec3 &end,
	...
	for ( i = 0, j = 0; i < tri->numIndexes; i += 3, j++ ) {
		...
		hit.materialType = R_GetMaterialType( shader, tri, j, point );
	}
...
void RB_ShowTrace( drawSurf_t **drawSurfs, int numDrawSurfs ) {
	...
	for ( i = 0 ; i < numDrawSurfs ; i++ ) {
		...
		hit = R_LocalTrace( localStart, localEnd, radius, tri, surf->material );
		if ( hit.fraction < 1.0 ) {
			qglColor4f( 1, 1, 1, 1 );
