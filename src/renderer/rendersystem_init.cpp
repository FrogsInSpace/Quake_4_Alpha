...
idCVar r_skipTextures( "r_skipTextures", "0", CVAR_RENDERER, "fixme" );
idCVar r_useSimpleInteraction( "r_useSimpleInteraction", "0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL,
	"use simple interaction program for faster rendering", 0, 1, idCmdSystem::ArgCompletion_Boolean );
idCVar r_showEditorImages( "r_showEditorImages", "0", CVAR_RENDERER,
	"renders the editor image rather than the texture" );
idCVar r_portalsDistanceCull( "r_portalsDistanceCull", "1", CVAR_RENDERER | CVAR_BOOL,
	"Allow culling of portals based on distance to viewer" );
idCVar r_alphaToCoverage( "r_alphaToCoverage", "1",
	CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL,
	"use alpha to coverage when multisampling" );
...
idCVar r_forceDiffuseOnly( "r_forceDiffuseOnly", "0", CVAR_RENDERER | CVAR_BOOL, "forces renderer to skip all but diffuse pass on non-arb2 renderers" );
...
// Raven's NV program entry points. These data symbols are owned by
// RenderSystem_init.obj alongside the other GL extension pointers.
void ( APIENTRY *qglBindProgramNV )( GLenum target, GLuint id );
void ( APIENTRY *qglLoadProgramNV )( GLenum target, GLuint id, GLsizei len, const GLubyte *program );
void ( APIENTRY *qglProgramParameter4fvNV )( GLenum target, GLuint index, const GLfloat *params );

void ( APIENTRY *qglBlendEquationEXT )( GLenum mode );
...
// Raven GLSL shader-stage entry points.
...
static void R_CheckPortableExtensions( void ) {
	...
	if ( !glConfig.multitextureAvailable || !glConfig.textureEnvCombineAvailable || !glConfig.cubeMapAvailable
		...
		common->Error( common->GetLocalizedString( "#str_06780" ) );
	}
...
void R_TestVideo_f( const idCmdArgs &args ) {
	...
	soundSystem->PlayShaderDirectly( 1, wavString.c_str(), -1 );
}
...
void idRenderSystemLocal::TakeScreenshot( int width, int height, const char *fileName, int blends, renderView_t *ref, const char *basePath ) {
	...
	fileSystem->WriteFile( fileName, buffer, c, basePath );
...
void idRenderSystemLocal::TakeJPGScreenshot( int width, int height, const char *fileName,
		int blends, renderView_t *ref, const char *basePath ) {
	// Keep screenshot capture available while the retail in-memory JPEG
	// destination manager is reconstructed. The pixel/readback path and blend
	// behavior are identical; TakeScreenshot supplies the stable fallback.
	TakeScreenshot( width, height, fileName, blends, ref, basePath );
}
...
================== 
*/  
void R_ScreenshotFilename( int &lastNumber, const char *base, const char *extension, idStr &fileName ) {
	int	a,b,c,d, e;
	...
	for ( ; lastNumber < 99999 ; lastNumber++ ) {
		...
		sprintf( fileName, "%s%i%i%i%i%i%s", base, a, b, c, d, e, extension );
		if ( lastNumber == 99999 ) {
			break;
		}
...
void R_ScreenShot_f( const idCmdArgs &args ) {
	...
	case 1:
		...
		blends = 1;
		R_ScreenshotFilename( lastNumber, "screenshots/shot", ".tga", checkname );
		break;
	case 2:
		width = glConfig.vidWidth;
		height = glConfig.vidHeight;
...
void idRenderSystemLocal::Clear( void ) {
	...
	smpFrame = 0;
	deltaTime = 0.0f;
	mBenchmarkMode = BENCHMARK_NONE;
	memset( &mSavePC, 0, sizeof( mSavePC ) );
	lastLoadedImage.Clear();
	viewportOffset[0] = 0;
	viewportOffset[1] = 0;
	tiledViewport[0] = 0;
	tiledViewport[1] = 0;
	...
	testImage = NULL;
	testVideo = NULL;
	memset( debugGraph, 0, sizeof( debugGraph ) );
	debugGraphMaterial = NULL;
	testStandaloneVideo = NULL;
...
void idRenderSystemLocal::Shutdown( void ) {	
	...
	// close the r_logFile
	if ( qglLogFile ) {
		fprintf( qglLogFile, "*** CLOSING LOG ***\n" );
		fclose( qglLogFile );
		qglLogFile = 0;
	}
