...
Quake 4 stores each point size in a single .fontdat record.  The trailing
material pointer is deliberately not read from disk; it is resolved from the
same name after the platform-independent float fields have been decoded.
====================
*/
bool idRenderSystemLocal::RegisterFont( const char *fontName, fontInfoEx_t &font ) {
	...
	for ( int fontIndex = 0; fontIndex < 3; fontIndex++ ) {
		...
		if ( fontIndex == 0 ) {
			outFont = &font.fontInfoSmall;
			maxWidth = &font.maxWidthSmall;
			maxHeight = &font.maxHeightSmall;
		} else if ( fontIndex == 1 ) {
			outFont = &font.fontInfoMedium;
			maxWidth = &font.maxWidthMedium;
			maxHeight = &font.maxHeightMedium;
		} else {
			outFont = &font.fontInfoLarge;
			maxWidth = &font.maxWidthLarge;
			maxHeight = &font.maxHeightLarge;
		}

		char fileName[MAX_OSPATH];
		idStr::snPrintf( fileName, sizeof( fileName ), "%s_%i.fontdat", fontName, pointSizes[fontIndex] );
		if ( fileSystem->ReadFile( fileName, NULL, NULL ) != sizeof( fontInfo_t ) ) {
			continue;
		}
		idFile *fontFile = fileSystem->OpenFileRead( fileName, true );
		if ( fontFile == NULL ) {
			continue;
		}

		for ( int glyphIndex = 0; glyphIndex < GLYPH_COUNT; glyphIndex++ ) {
			glyphInfo_t &glyph = outFont->glyphs[glyphIndex];
			fontFile->ReadFloat( glyph.width );
			fontFile->ReadFloat( glyph.height );
			fontFile->ReadFloat( glyph.horiAdvance );
			fontFile->ReadFloat( glyph.horiBearingX );
			fontFile->ReadFloat( glyph.horiBearingY );
			fontFile->ReadFloat( glyph.s1 );
			fontFile->ReadFloat( glyph.t1 );
			fontFile->ReadFloat( glyph.s2 );
			fontFile->ReadFloat( glyph.t2 );
			*maxWidth = Max( *maxWidth, glyph.horiAdvance );
			*maxHeight = Max( *maxHeight, glyph.height );
		}

		fontFile->ReadFloat( outFont->pointSize );
		fontFile->ReadFloat( outFont->fontHeight );
		fontFile->ReadFloat( outFont->ascender );
		fontFile->ReadFloat( outFont->descender );
		fileSystem->CloseFile( fontFile );

		outFont->material = const_cast<idMaterial *>( declManager->FindMaterial( fileName ) );
	...
	switch ( found ) {
		case 1:
			font.fontInfoMedium = font.fontInfoSmall;
		...
		case 3:
			...
			break;
		case 4:
			font.fontInfoSmall = font.fontInfoLarge;
			font.maxHeightSmall = font.maxHeightLarge;
			font.fontInfoMedium = font.fontInfoLarge;
