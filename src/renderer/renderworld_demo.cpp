...
bool idRenderWorldLocal::ProcessDemoCommand( idDemoFile *readDemo, renderView_t *renderView,
		renderView_t *portalRenderView, int *demoTimeOffset ) {
	const bool endFrame = ProcessDemoCommand( readDemo, renderView, demoTimeOffset );
	if ( portalRenderView != NULL && renderView != NULL ) {
...
void idRenderWorldLocal::WriteRenderLight( idDemoFile *demo, const renderLight_t *light ) {
	if ( demo == NULL || light == NULL ) {
		return;
	}
	demo->WriteMat3( light->axis );
	demo->WriteVec3( light->origin );
	demo->WriteInt( light->suppressLightInViewID );
	demo->WriteInt( light->allowLightInViewID );
	demo->WriteBool( light->noShadows );
	demo->WriteBool( light->noSpecular );
	demo->WriteBool( light->pointLight );
	demo->WriteBool( light->parallel );
	demo->WriteVec3( light->lightRadius );
	demo->WriteVec3( light->lightCenter );
	demo->WriteVec3( light->target );
	demo->WriteVec3( light->right );
	demo->WriteVec3( light->up );
	demo->WriteVec3( light->start );
	demo->WriteVec3( light->end );
	demo->WriteInt( light->lightId );
	for ( int i = 0; i < MAX_ENTITY_SHADER_PARMS; ++i ) {
		demo->WriteFloat( light->shaderParms[i] );
	}
	demo->WriteInt( light->referenceSoundHandle );
	demo->WriteHashString( light->prelightModel ? light->prelightModel->Name() : "" );
	demo->WriteHashString( light->shader ? light->shader->GetName() : "" );
}
...
void idRenderWorldLocal::ReadRenderLight( idDemoFile *demo, renderLight_t &light ) {
	memset( &light, 0, sizeof( light ) );
	if ( demo == NULL ) {
		return;
	}
	demo->ReadMat3( light.axis );
	demo->ReadVec3( light.origin );
	demo->ReadInt( light.suppressLightInViewID );
	demo->ReadInt( light.allowLightInViewID );
	demo->ReadBool( light.noShadows );
	demo->ReadBool( light.noSpecular );
	demo->ReadBool( light.pointLight );
	demo->ReadBool( light.parallel );
	demo->ReadVec3( light.lightRadius );
	demo->ReadVec3( light.lightCenter );
	demo->ReadVec3( light.target );
	demo->ReadVec3( light.right );
	demo->ReadVec3( light.up );
	demo->ReadVec3( light.start );
	demo->ReadVec3( light.end );
	demo->ReadInt( light.lightId );
	for ( int i = 0; i < MAX_ENTITY_SHADER_PARMS; ++i ) {
		demo->ReadFloat( light.shaderParms[i] );
	}
	demo->ReadInt( light.referenceSoundHandle );
	const char *prelightName = demo->ReadHashString();
	if ( prelightName != NULL && prelightName[0] != '\0' ) {
		light.prelightModel = renderModelManager->FindModel( prelightName );
	}
	const char *shaderName = demo->ReadHashString();
	if ( shaderName != NULL && shaderName[0] != '\0' ) {
		light.shader = declManager->FindMaterial( shaderName );
	}
}
