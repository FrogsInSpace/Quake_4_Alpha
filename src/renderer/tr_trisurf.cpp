...
static idBlockAlloc<srfTriangles_t, 1<<8, MA_RENDER>		srfTrianglesAllocator;

#ifdef USE_TRI_DATA_ALLOCATOR
static idDynamicBlockAlloc<idDrawVert, 1<<20, 1<<10, MA_RENDER>	triVertexAllocator;
static idDynamicBlockAlloc<glIndex_t, 1<<18, 1<<10, MA_RENDER>		triIndexAllocator;
static idDynamicBlockAlloc<shadowCache_t, 1<<18, 1<<10, MA_RENDER>	triShadowVertexAllocator;
static idDynamicBlockAlloc<idPlane, 1<<17, 1<<10, MA_RENDER>		triPlaneAllocator;
static idDynamicBlockAlloc<glIndex_t, 1<<17, 1<<10, MA_RENDER>		triSilIndexAllocator;
static idDynamicBlockAlloc<silEdge_t, 1<<17, 1<<10, MA_RENDER>		triSilEdgeAllocator;
static idDynamicBlockAlloc<dominantTri_t, 1<<16, 1<<10, MA_RENDER>	triDominantTrisAllocator;
static idDynamicBlockAlloc<int, 1<<16, 1<<10, MA_RENDER>			triMirroredVertAllocator;
static idDynamicBlockAlloc<int, 1<<16, 1<<10, MA_RENDER>			triDupVertAllocator;
static idDynamicBlockAlloc<rvSilTraceVertT, 1<<18, 1<<10, MA_RENDER>	silTraceVertexAllocator;
static idDynamicBlockAlloc<float, 1<<16, 1<<10, MA_RENDER>		skinToModelTransformAllocator;
#else
static idDynamicAlloc<idDrawVert, 1<<20, 1<<10, MA_RENDER>			triVertexAllocator;
static idDynamicAlloc<glIndex_t, 1<<18, 1<<10, MA_RENDER>			triIndexAllocator;
static idDynamicAlloc<shadowCache_t, 1<<18, 1<<10, MA_RENDER>		triShadowVertexAllocator;
static idDynamicAlloc<idPlane, 1<<17, 1<<10, MA_RENDER>			triPlaneAllocator;
static idDynamicAlloc<glIndex_t, 1<<17, 1<<10, MA_RENDER>			triSilIndexAllocator;
static idDynamicAlloc<silEdge_t, 1<<17, 1<<10, MA_RENDER>			triSilEdgeAllocator;
static idDynamicAlloc<dominantTri_t, 1<<16, 1<<10, MA_RENDER>		triDominantTrisAllocator;
static idDynamicAlloc<int, 1<<16, 1<<10, MA_RENDER>				triMirroredVertAllocator;
static idDynamicAlloc<int, 1<<16, 1<<10, MA_RENDER>				triDupVertAllocator;
static idDynamicAlloc<rvSilTraceVertT, 1<<18, 1<<10, MA_RENDER>		silTraceVertexAllocator;
static idDynamicAlloc<float, 1<<16, 1<<10, MA_RENDER>			skinToModelTransformAllocator;
#endif
...
void R_ReallyFreeStaticTriSurf( srfTriangles_t *tri ) {
	...
	if ( tri->silTraceVertsAlloc != NULL ) {
		silTraceVertexAllocator.Free( tri->silTraceVertsAlloc );
	}
...
void R_AllocStaticTriSurfSilTraceVerts( srfTriangles_t *tri, int numVerts ) {
	assert( tri->silTraceVertsAlloc == NULL );
...
void R_AllocStaticSkinToModelTransforms( srfTriangles_t *tri, int numTransforms ) {
	...
	tri->skinToModelTransformsAlloc = skinToModelTransformAllocator.Alloc(
		numTransforms * 16 );
