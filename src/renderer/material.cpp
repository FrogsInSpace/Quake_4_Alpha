...
int idMaterial::GetTexelCount() const {
	int count = 0;
	for ( int i = 0; stages && i < numStages; ++i ) {
		if ( stages[i].texture.image ) {
			const idImage *image = stages[i].texture.image;
			count += image->uploadWidth * image->uploadHeight * Max( image->uploadDepth, 1 );
		}
	}
	return count;
}
...
const rvDeclMatType *idMaterial::GetMaterialType( idVec2 &tc ) const {
	if ( !materialTypeArray || MTAWidth <= 0 || MTAHeight <= 0 ) {
		return materialType;
	}
	tc.x = fmodf( tc.x, 1.0f );
	tc.y = fmodf( tc.y, 1.0f );
	if ( tc.x < 0.0f ) tc.x += 1.0f;
	if ( tc.y < 0.0f ) tc.y += 1.0f;
	const int x = idMath::ClampInt( 0, MTAWidth - 1, idMath::Ftoi( tc.x * MTAWidth ) );
	const int y = idMath::ClampInt( 0, MTAHeight - 1, idMath::Ftoi( tc.y * MTAHeight ) );
	const int index = materialTypeArray[x + y * MTAWidth];
	const rvDeclMatType *type = static_cast<const rvDeclMatType *>(
		declManager->DeclByIndex( DECL_MATERIALTYPE, index, false ) );
	return type ? type : materialType;
}

bool idMaterial::Validate( const char *text, int textLength, idStr &report ) const {
	(void)report;
	if ( textLength <= 0 ) {
		return false;
	}
	idDecl *decl = declManager->AllocateDecl( DECL_MATERIAL );
	const bool valid = decl && decl->Parse( text, textLength, false );
	if ( decl ) {
		decl->FreeData();
		delete decl->base;
		delete decl;
...
void idMaterial::EvaluateRegisters( float *registers, const float shaderParms[MAX_ENTITY_SHADER_PARMS],
									const viewDef_t *view, int soundEmitter, idVec3 *randomizer ) const {
	int		i, b;
	expOp_t	*op;
	...
	registers[EXP_REG_VERTEX_RANDOMIZER] = randomizer ? randomizer->x + randomizer->y + randomizer->z : 0.0f;
	...
	for ( i = 0 ; i < numOps ; i++, op++ ) {
		...
		case OP_TYPE_SOUND:
			registers[op->c] = 0.0f;
			if ( soundEmitter ) {
				idSoundEmitter *emitter = soundSystem->EmitterForIndex( -1, soundEmitter );
				if ( emitter ) {
...
void idMaterial::EvaluateStageRegisters( int stageIndex, float *registers,
	...
	if ( !registers || !shaderParms || stageIndex < 0 || stageIndex >= numStages ) {
		return;
	}
	// The retail routine limits evaluation to this stage's operation span.
	// Evaluating the complete expression program preserves shared dependencies
	// and produces the same values for the requested stage registers.
	viewDef_t localView;
	memset( &localView, 0, sizeof( localView ) );
	localView.floatTime = floatTime;
	EvaluateRegisters( registers, shaderParms, &localView, 0, NULL );
}
