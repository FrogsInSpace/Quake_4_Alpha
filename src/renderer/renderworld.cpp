...
==================
*/
bool idRenderWorldLocal::EffectDefHasSound( const renderEffect_t *reffect ) {
	return reffect != NULL && bse != NULL && bse->CheckDefForSound( reffect );
}
...
qhandle_t idRenderWorldLocal::AddEffectDef( const renderEffect_t *reffect, int time ) {
	if ( !bse_enabled.GetBool() || reffect == NULL ) {
		return -1;
	}

	int handle = effectDefs.FindNull();
	if ( handle == -1 ) {
		handle = effectDefs.Append( NULL );
	}

	// The retail interface returns true from UpdateEffectDef when the
	// effect has already completed or could not be created.
	if ( UpdateEffectDef( handle, reffect, time ) ) {
		FreeEffectDef( handle );
		return -1;
	}
	return handle;
}
...
/*
==================
FreeEffectDefDerivedData
==================
*/
void idRenderWorldLocal::FreeEffectDefDerivedData( rvRenderEffectLocal *def ) {
	if ( def == NULL ) {
		return;
	}
	for ( areaReference_t *ref = def->effectRefs; ref != NULL; ) {
		areaReference_t *next = ref->ownerNext;
		ref->areaNext->areaPrev = ref->areaPrev;
		ref->areaPrev->areaNext = ref->areaNext;
		areaReferenceAllocator.Free( ref );
		ref = next;
	}
	def->effectRefs = NULL;
	def->viewEffect = NULL;
...
==================
*/
void idRenderWorldLocal::AddEffectRefToArea( rvRenderEffectLocal *def, portalArea_t *area ) {
	if ( def == NULL || area == NULL ) {
		return;
	}
	...
	ref->effect = def;
	ref->area = area;
	ref->ownerNext = def->effectRefs;
	def->effectRefs = ref;

	ref->areaNext = &area->effectRefs;
	ref->areaPrev = area->effectRefs.areaPrev;
...
bool idRenderWorldLocal::UpdateEffectDef( qhandle_t effectHandle, const renderEffect_t *reffect, int time ) {
	...
	if ( !bse_enabled.GetBool() || bse == NULL || reffect == NULL ) {
		return true;
	}
	...
	if ( def != NULL ) {
		// The retail executable compares the placement portion (0x70 bytes)
		// before retaining the existing area references.
		const int placementBytes = Min( static_cast<int>( sizeof( renderEffect_t ) ), 0x70 );
		if ( memcmp( reffect, &def->parms, placementBytes ) != 0 ) {
			FreeEffectDefDerivedData( def );
			push = true;
		}
		def->parms = *reffect;
	} else {
		def = new rvRenderEffectLocal;
		effectDefs[effectHandle] = def;
		def->index = effectHandle;
		def->world = this;
		def->parms = *reffect;
		push = true;
		if ( !bse->PlayEffect( def, time * 0.001f ) ) {
			return true;
		}
	}
	...
	def->lastModifiedFrameNum = tr.frameCount;
	R_AxisToModelMatrix( def->parms.axis, def->parms.origin, def->modelMatrix );
	if ( bse->ServiceEffect( def, time * 0.001f ) ) {
		return true;
	}
	if ( push ) {
		PushEffectDef( def );
...
void idRenderWorldLocal::StopEffectDef( qhandle_t effectHandle ) {
	if ( effectHandle < 0 || effectHandle >= effectDefs.Num() ) {
		common->Printf( "idRenderWorld::StopEffectDef: invalid handle %i >= %i\n", effectHandle, effectDefs.Num() );
		return;
	}
	if ( effectDefs[effectHandle] != NULL && bse != NULL ) {
...
const rvRenderEffectLocal *idRenderWorldLocal::GetEffectDef( qhandle_t effectHandle ) const {
	if ( effectHandle < 0 || effectHandle >= effectDefs.Num() ) {
		common->Printf( "idRenderWorld::GetEffectDef: invalid handle %i >= %i\n", effectHandle, effectDefs.Num() );
		return NULL;
	}
...
void idRenderWorldLocal::FreeEffectDef( qhandle_t effectHandle ) {
	if ( effectHandle < 0 || effectHandle >= effectDefs.Num() ) {
		common->Printf( "idRenderWorld::FreeEffectDef: invalid handle %i >= %i\n", effectHandle, effectDefs.Num() );
		return;
	}
	rvRenderEffectLocal *def = effectDefs[effectHandle];
	if ( def == NULL ) {
...
void idRenderWorldLocal::PushMarkedDefs() {
...
void idRenderWorldLocal::ClearMarkedDefs() {
	markedEntityDefs.Clear();
	markedLightDefs.Clear();
	markedEffectDefs.Clear();
}

void idRenderWorldLocal::RemoveAllModelReferences( idRenderModel *model ) {
	for ( int i = localModels.Num() - 1; i >= 0; --i ) {
		if ( localModels[i] == model ) {
...
void idRenderWorldLocal::RenderScene( const renderView_t *renderView ) {
	RenderScene( renderView, 0 );
}

bool idRenderWorldLocal::HasSkybox( int areaNum ) {
	return areaNum >= 0 && areaNum < numPortalAreas && portalAreas[areaNum].hasSkybox;
}

static void R_WorldToScreenMatrices( const renderView_t *renderView, float modelView[16], float projection[16] ) {
	viewDef_t viewDef;
	memset( &viewDef, 0, sizeof( viewDef ) );
	viewDef.renderView = *renderView;
	R_SetViewMatrix( &viewDef );
	memcpy( modelView, viewDef.worldSpace.modelViewMatrix, sizeof( viewDef.worldSpace.modelViewMatrix ) );

	memset( projection, 0, sizeof( float ) * 16 );
	const float zNear = r_znear.GetFloat() * ( renderView->cramZNear ? 0.25f : 1.0f );
	const float xmax = zNear * idMath::Tan16( DEG2RAD( renderView->fov_x * 0.5f ) );
	const float ymax = zNear * idMath::Tan16( DEG2RAD( renderView->fov_y * 0.5f ) );
	projection[0] = zNear / xmax;
	projection[5] = zNear / ymax;
	projection[10] = -0.999f;
	projection[14] = -2.0f * zNear;
	projection[11] = -1.0f;
}

idVec3 idRenderWorldLocal::WorldToScreen( renderView_t *renderView, idVec3 worldPoint ) {
	float modelView[16], projection[16];
	idVec3 ndc;
	R_WorldToScreenMatrices( renderView, modelView, projection );
	R_GlobalToNormalizedDeviceCoordinates( modelView, projection, worldPoint, ndc );
	return idVec3( SCREEN_WIDTH * ( ndc.x + 1.0f ) * 0.5f,
		SCREEN_HEIGHT - SCREEN_HEIGHT * ( ndc.y + 1.0f ) * 0.5f, ndc.z );
}

idBounds idRenderWorldLocal::WorldToScreen( renderView_t *renderView, idBounds worldBounds ) {
	idVec3 points[8];
	worldBounds.ToPoints( points );
	idBounds screenBounds;
	screenBounds.Clear();
	for ( int i = 0; i < 8; ++i ) {
		screenBounds.AddPoint( WorldToScreen( renderView, points[i] ) );
	}
	return screenBounds;
}
...
void idRenderWorldLocal::GetPortals( int areaNum, exitPortal_t *ret, int maxPortals ) {
	if ( ret == NULL || maxPortals <= 0 || areaNum < 0 || areaNum >= numPortalAreas ) {
		return;
	}
	int count = 0;
	for ( portal_t *portal = portalAreas[areaNum].portals; portal != NULL && count < maxPortals; portal = portal->next, ++count ) {
		ret[count].areas[0] = areaNum;
		ret[count].areas[1] = portal->intoArea;
		ret[count].w = portal->w;
		ret[count].blockingBits = portal->doublePortal->blockingBits;
		ret[count].portalHandle = static_cast<int>( portal->doublePortal - doublePortals ) + 1;
	}
}
...
guiPoint_t	idRenderWorldLocal::GuiTrace( qhandle_t entityHandle, const idVec3 start, const idVec3 end ) const {
	...
	for ( j = 0 ; j < model->NumSurfaces() ; j++ ) {
		...
		local = R_LocalTrace( localStart, localEnd, 0.0f, tri, shader );
...
//		common->Error( "idRenderWorld::ModelTrace: index = %i", entityHandle );
	...
	for ( i = 0; i < model->NumBaseSurfaces(); i++ ) {
		...
		if ( localTrace.fraction < trace.fraction ) {
			...
			trace.materialType = localTrace.materialType;
...
void idRenderWorldLocal::DebugBounds( const idVec4 &color, const idBounds &bounds,
		const idVec3 &org, const int lifetime, bool depthTest ) {
	if ( bounds.IsCleared() ) {
		return;
	}
	idVec3 v[8];
...
size_t idRenderWorldLocal::MemorySummary( const idCmdArgs &args ) {
	...
	size_t bytes = areaReferenceAllocator.Allocated() + interactionAllocator.Allocated() +
		areaNumRefAllocator.Allocated() + entityDefs.Allocated() + lightDefs.Allocated() +
		effectDefs.Allocated() + localModels.Allocated();
	for ( int i = 0; i < localModels.Num(); ++i ) {
		if ( localModels[i] != NULL ) {
			bytes += localModels[i]->Memory();
		}
	}
	const size_t kb = bytes >> 10;
	common->Printf( "Renderer        - %uK\n", static_cast<unsigned int>( kb ) );
	return kb;
}

void idRenderWorldLocal::DebugFOV( const idVec4 &color, const idVec3 &origin,
		const idVec3 &direction, float farDot, float farDist, float nearDot,
		float nearDist, float alpha, int lifetime ) {
	...
	farDot = idMath::ClampFloat( -1.0f, 1.0f, farDot );
	nearDot = idMath::ClampFloat( -1.0f, 1.0f, nearDot );
	const float farAngle = idMath::ACos( farDot );
	const float nearAngle = idMath::ACos( nearDot );
	const float farRadius = farDist * idMath::Tan16( farAngle );
	const float nearRadius = nearDist * idMath::Tan16( nearAngle );
	const idVec3 farCenter = origin + dir * farDist;
	const idVec3 nearCenter = origin + dir * nearDist;
	idVec4 drawColor = color;
	if ( alpha < 1.0f ) {
		drawColor.w = alpha;
	}
	DebugCircle( drawColor, farCenter, dir, farRadius, 32, lifetime, true );
	if ( nearDist > 0.0f ) {
		DebugCircle( drawColor, nearCenter, dir, nearRadius, 32, lifetime, true );
	}
	for ( int i = 0; i < 4; ++i ) {
		const float a = idMath::HALF_PI * i;
		const idVec3 radial = right * idMath::Cos16( a ) + up * idMath::Sin16( a );
		DebugLine( drawColor, nearCenter + radial * nearRadius,
			farCenter + radial * farRadius, lifetime, true );
	}
}
