...
=================
*/
#define R_FinishDeform( drawSurf, newTri, ac ) do { \
	if ( (drawSurf)->material->ReceivesLighting() ) { \
		(newTri)->verts = (ac); \
		R_DeriveTangents( (newTri), false ); \
		(newTri)->verts = NULL; \
	} \
	(newTri)->ambientCache = vertexCache.AllocFrameTemp( (ac), (newTri)->numVerts * sizeof( idDrawVert ), 2 ); \
	if ( (newTri)->ambientCache ) { \
		(drawSurf)->geo = (newTri); \
	} \
	if ( r_useIndexBuffers.GetBool() ) { \
		(newTri)->indexCache = vertexCache.AllocFrameTemp( (newTri)->indexes, \
			(newTri)->numIndexes * sizeof( (newTri)->indexes[0] ), 1 ); \
	} \
} while ( 0 )
...
static void R_SpriteDeform( drawSurf_t *surf, bool radiusDimension ) {
	...
	for ( i = 0 ; i < tri->numVerts ; i+=4 ) {
		idVec3 positions[4];
		if ( tri->silTraceVerts ) {
			for ( int j = 0; j < 4; ++j ) {
				positions[j].Set( tri->silTraceVerts[i+j].xyzw[0],
					tri->silTraceVerts[i+j].xyzw[1], tri->silTraceVerts[i+j].xyzw[2] );
			}
		} else {
			for ( int j = 0; j < 4; ++j ) {
				positions[j] = tri->verts[i+j].xyz;
			}
		}

		if ( radiusDimension ) {
			mid = ( positions[0] + positions[1] + positions[2] + positions[3] ) * 0.25f;
			radius = ( positions[0] - mid ).Length() * 0.707f;
			radius2 = radius;
		} else {
			// The retail rectangular-sprite path intentionally leaves mid unchanged.
			radius = ( positions[2] - positions[0] ).Length() * 0.5f;
			radius2 = ( positions[1] - positions[0] ).Length() * 0.5f;
		}
...
static int edgeVerts[6][2] = {
	...
	if ( tri->primBatchMesh ) {
		newTri->numVerts = tri->primBatchMesh->m_numDrawVertices;
		newTri->numIndexes = tri->primBatchMesh->m_numDrawIndices;
		newTri->indexes = (glIndex_t *)R_FrameAlloc( newTri->numIndexes * sizeof( newTri->indexes[0] ) );
		ac = (idDrawVert *)_alloca16( newTri->numVerts * sizeof( idDrawVert ) );
		memset( ac, 0, sizeof( idDrawVert ) * newTri->numVerts );
		tri->primBatchMesh->TubeDeform( ac, newTri->indexes, localView, tri->silTraceVerts );
	} else {
		newTri->numVerts = tri->numVerts;
		newTri->numIndexes = tri->numIndexes;
		newTri->indexes = (glIndex_t *)R_FrameAlloc( newTri->numIndexes * sizeof( newTri->indexes[0] ) );
...
static void R_FlareDeform( drawSurf_t *surf ) {
	...
	if ( tri->primBatchMesh ) {
		...
		tempTri.numVerts = tri->primBatchMesh->m_numDrawVertices;
		tempTri.numIndexes = tri->primBatchMesh->m_numDrawIndices;
		tempTri.indexes = (glIndex_t *)R_FrameAlloc( tempTri.numIndexes * sizeof( tempTri.indexes[0] ) );
		tempTri.verts = (idDrawVert *)_alloca16( tempTri.numVerts * sizeof( idDrawVert ) );
		tri->primBatchMesh->CopyTriangles( tempTri.verts, tempTri.indexes, tri->silTraceVerts );
		tri = &tempTri;
	}
...
static void R_ExpandDeform( drawSurf_t *surf ) {
	...
	if ( tri->primBatchMesh ) {
		...
		newTri->indexes = (glIndex_t *)R_FrameAlloc( newTri->numIndexes * sizeof( newTri->indexes[0] ) );
		ac = (idDrawVert *)_alloca16( newTri->numVerts * sizeof( idDrawVert ) );
		tri->primBatchMesh->CopyTriangles( ac, newTri->indexes, tri->silTraceVerts );

		// Static vertex buffers do not necessarily contain derived tangents/normals.
		newTri->verts = ac;
		R_DeriveTangents( newTri, false );
		newTri->verts = NULL;
