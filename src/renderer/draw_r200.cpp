...
//		common->Printf( "Note: Shader %s drawn as two pass on R200\n", din->surf->shader->getName() );
		...
		SIMDProcessor->Memcpy( d.diffuseMatrix, d.specularMatrix, sizeof( d.diffuseMatrix ) );
	...
	case SVC_IGNORE:
		qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, PP_COLOR_MODULATE, zeroOne );
		break;
	case SVC_MODULATE:
		qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, PP_COLOR_MODULATE, oneZero );
		break;
	case SVC_INVERSE_MODULATE:
		qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, PP_COLOR_MODULATE, negOneOne );
		break;
	}
...
static void RB_R200_ARB_CreateDrawInteractions( const drawSurf_t *surf ) {
	...
	GL_EnableVertexAttribState( 0x30 );
	GL_SelectTexture( 3 );
	GL_EnableVertexAttribState( 0x40 );
	...
	GL_SelectTexture( 3 );
	globalImages->BindNull();
	GL_DisableVertexAttribState( 0x40 );

	GL_SelectTexture( 2 );
	globalImages->BindNull();
	GL_DisableVertexAttribState( 0x20 );
