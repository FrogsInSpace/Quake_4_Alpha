...
==============
*/
bool idCinematic::InitFromFile( const char *qpath, bool looping, int options ) {
	Close();

	// The retail executable selects a private decoder from the extension.
	// WMV playback depended on the original Windows Media SDK; the shipped
	// startup and material cinematics are RoQ and use the in-tree decoder.
	if ( idStr::FindText( qpath, ".wmv", false ) >= 0 ) {
		delete PIMPL;
		PIMPL = NULL;
		mode = 0;
		return false;
	...
	delete PIMPL;
	PIMPL = new idCinematicLocal;
	mode = 1;
	return PIMPL->InitFromFile( qpath, looping, options );
}
...
int idCinematic::AnimationLength() {
	return PIMPL ? PIMPL->AnimationLength() : 0;
}
...
cinData_t idCinematic::ImageForTime( int milliseconds ) {
	if ( PIMPL ) {
		return PIMPL->ImageForTime( milliseconds );
	}
	cinData_t data;
...
void idCinematic::Draw() {
	if ( PIMPL ) {
		PIMPL->Draw();
	}
}

void idCinematic::SetScreenRect( int left, int right, int bottom, int top ) {
	if ( PIMPL ) {
		PIMPL->SetScreenRect( left, right, bottom, top );
	}
}

void idCinematic::GetScreenRect( int &left, int &right, int &bottom, int &top ) {
	if ( PIMPL ) {
		PIMPL->GetScreenRect( left, right, bottom, top );
	} else {
		left = right = bottom = top = 0;
	}
}

bool idCinematic::IsPlaying() {
	return PIMPL ? PIMPL->IsPlaying() : false;
}
...
==============
*/
bool idCinematicLocal::InitFromFile( const char *qpath, bool amilooping, int options ) {
	...
	if ( idStr::Icmp( fileName.Right( 4 ), ".roq" ) != 0 ) {
		fileName += ".roq";
	}
...
void idCinematicLocal::ResetTime(int time) {
	// Quake 4 deliberately defers the new epoch until the next
	// ImageForTime call.  GUI scripts reset cinematics while the front end is
	// assembling a view, so using the previous backend view time here can put
	// a freshly-reset movie ahead of (or behind) its GUI timeline.
	startTime = -1;
	status = FMV_PLAY;
}
...
==============
*/
bool idSndWindow::InitFromFile( const char *qpath, bool looping, int options ) {
	(void)looping;
	(void)options;
	idStr fname = qpath;
