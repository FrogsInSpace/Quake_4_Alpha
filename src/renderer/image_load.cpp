...
void idPBufferImage::Bind() {
	if ( mRenderTarget != NULL ) {
		mRenderTarget->BeginTexture( texnum, GL_NEAREST, GL_NEAREST, GL_CLAMP_TO_EDGE );
	}
}

void idPBufferImage::UnBind() {
	if ( mRenderTarget != NULL ) {
		mRenderTarget->EndTexture();
	}
}

void idPBufferImage::PurgeImage() {
	if ( mRenderTarget != NULL ) {
...
GLenum idImage::SelectInternalFormat( const byte **dataPtrs, int numDataPtrs, int width, int height,
	...
	*monochromeResult = true;	// until shown otherwise
	...
	if ( minimumDepth == TD_BUMP ) {
		...
		} else if ( globalImages->image_useCompression.GetBool() && globalImages->image_useNormalCompression.GetInteger() ) {
