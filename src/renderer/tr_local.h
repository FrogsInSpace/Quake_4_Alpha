...
typedef struct areaReference_s {
	...
	rvRenderEffectLocal *	effect;					// Quake 4 BSE effect reference
...
public:
	virtual					~rvRenderEffect() {}
	virtual void			FreeRenderEffect() = 0;
	virtual void			UpdateRenderEffect( const renderEffect_t *effect, bool forceUpdate = false ) = 0;
	virtual void			ForceUpdate() = 0;
	virtual int				GetIndex() = 0;
};
...
public:
						rvRenderEffectLocal();
	virtual				~rvRenderEffectLocal();
	virtual void			FreeRenderEffect();
	virtual void			UpdateRenderEffect( const renderEffect_t *effect, bool forceUpdate = false );
...
typedef struct viewEntity_s {
	...
	union {
		...
		bool			weaponDepthHack;	// transitional source alias; occupies the target's four-byte slot
	};
...
typedef struct {
	...
	int		c_entityPushes;
	int		c_entityPushesSuppressed;
	int		c_lightUpdates;
	int		c_lightReferences;
	int		c_lightPushes;
	int		c_lightPushesSuppressed;
	int		c_effectUpdates;
	int		c_effectReferences;
	int		c_effectPushes;
	int		c_effectPushesSuppressed;
	int		c_visibleViewEffects;
	int		c_culledViewEffects;
	int		c_guiSurfs;
	int		frontEndMsec;		// sum of time in all RE_RenderScene's in a frame
	int		c_numDecalIndexes;
	int		c_numVolumePushes;
} performanceCounters_t;
...
typedef struct {
	...
	unsigned int vertexAttribStateBits;
} glstate_t;
...
public:
	...
	virtual void			TakeJPGScreenshot( int width, int height, const char *fileName, int blends, renderView_t *ref, const char *basePath = "fs_savepath" );
	virtual void			TakeScreenshot( int width, int height, const char *fileName, int blends, renderView_t *ref, const char *basePath = "fs_savepath" );
...
public:
	...
	int						smpFrame;
	float					deltaTime;
	EBenchmarkType			mBenchmarkMode;
	performanceCounters_t	mSavePC;
	idStr					lastLoadedImage;
	...
	unsigned char			debugGraph[4][256][64];
	const idMaterial *		debugGraphMaterial;
	idCinematic *			testStandaloneVideo;
	...
	class idGuiModel *		guiOverlayModel;

	unsigned int			specialEffectsEnabled;
...
extern idCVar image_writeProgramImages;
...
extern idCVar r_skipDecals;				// 1 = don't render decals
...
extern idCVar r_skipTextures;			// use default/black/flat images instead of stage textures
extern idCVar r_useSimpleInteraction;
extern idCVar r_showEditorImages;
extern idCVar r_portalsDistanceCull;
extern idCVar r_alphaToCoverage;
...
extern idCVar r_forceDiffuseOnly;	// only draw diffuse passes on non-ARB2 paths
...
void	GL_PolygonOffsetState( bool enable, float factor, float units );
void	GL_PolygonOffset( const idMaterial *material, bool enable );
void	GL_VertexAttribState( unsigned int vertexAttribStateBits );
void	GL_EnableVertexAttribState( unsigned int vertexAttribStateBits );
void	GL_DisableVertexAttribState( unsigned int vertexAttribStateBits );
void	GL_DisableTexCoordState();
...
void R_GlobalToNormalizedDeviceCoordinates( float *modelViewMatrix, float *projectionMatrix, const idVec3 &global, idVec3 &ndc );
...
void RB_T_ShowBatchSizeRenderTriangleSurface( const drawSurf_t *surf );
void RB_T_ShowTriRenderTriangleSurface( const drawSurf_t *surf );
void RB_T_RenderSimpleSurface( const drawSurf_t *surf );
void RB_T_RenderTriangleSurfacePasses( const drawSurf_t *surf );
...
void RB_FinishStageTexturing( const shaderStage_t *pStage, const drawSurf_t *surf, idDrawVert *ac, bool fillingDepth );
void RB_GetCurrentRender( bool force );
void RB_BindVariableStageImage( const textureStage_t *texture );
void RB_BindStageTexture( const float *shaderRegisters, const textureStage_t *texture, const drawSurf_t *surf );
void RB_FinishStageTexture( const textureStage_t *texture, const drawSurf_t *surf );
void RB_StencilShadowPass( const drawSurf_t *drawSurfs );
int RB_STD_DrawShaderPasses( drawSurf_t **drawSurfs, int numDrawSurfs, materialSort_t minSort, materialSort_t maxSort );
...
typedef enum {
	...
	VPROG_MD5R_INTERACTION4,
	VPROG_MD5R_INTERACTION1,
	...
	VPROG_MD5R_SKYBOX1,
	VPROG_MD5R_SKYBOX,
	VPROG_MD5R_ENVNORMAL4,
	VPROG_MD5R_ENVNORMAL1,
	VPROG_MD5R_ENVNORMAL,
	VPROG_MD5R_ENVREFLECT4,
	VPROG_MD5R_ENVREFLECT1,
	VPROG_MD5R_ENVREFLECT,
	VPROG_MD5R_ENVBUMP4,
	VPROG_MD5R_ENVBUMP1,
	VPROG_MD5R_ENVBUMP,
	VPROG_MD5R_SHADOW4,
	VPROG_MD5R_SHADOW1,
	VPROG_MD5R_SHADOW,
	VPROG_MD5R_BASICFOG4,
	VPROG_MD5R_BASICFOG1,
	VPROG_MD5R_BASICFOG,
	VPROG_DEPTH,
	FPROG_DEPTH,
	FPROG_DEPTH_COC,
...
srfTriangles_t *R_CreateShadowVolume( const idRenderEntityLocal *ent,
									 ...
									 int optimize, srfCullInfo_t &cullInfo );
...
idScreenRect R_ScreenRectFromDrawSurf( drawSurf_t *drawSurf );
...
const rvDeclMatType *R_GetMaterialType( const idMaterial *shader,
	const srfTriangles_t *tri, int triangleNum, idVec3 &point );
localTrace_t R_LocalTrace( const idVec3 &start, const idVec3 &end,
	const float radius, const srfTriangles_t *tri, const idMaterial *shader );
