...
================
*/
void idGuiModel::EmitSurface( guiModelSurface_t *surf, float modelMatrix[16], float modelViewMatrix[16], int depthHackInViewID, float *shaderParms ) {
	srfTriangles_t	*tri;
	...
	if ( r_useIndexBuffers.GetBool() ) {
		tri->indexCache = vertexCache.AllocFrameTemp( &indexes[surf->firstIndex], tri->numIndexes * sizeof( tri->indexes[0] ), 1 );
	}
	...
	guiSpace->weaponDepthHackInViewID = depthHackInViewID;

	// add the surface, which might recursively create another gui
	R_AddDrawSurf( tri, guiSpace, &renderEntity, surf->material, tr.viewDef->scissor, this == tr.guiModel ? 2 : 0 );
	(void)shaderParms;
}
...
====================
*/
void idGuiModel::EmitToCurrentView( float modelMatrix[16], int depthHackInViewID, float *shaderParms ) {
	...
	for ( int i = 0 ; i < surfaces.Num() ; i++ ) {
		EmitSurface( &surfaces[i], modelMatrix, modelViewMatrix, depthHackInViewID, shaderParms );
	}
}
...
void idGuiModel::EmitFullScreen( void ) {
	...
	// for gui editor
	if ( !tr.viewDef || !( tr.viewDef->renderFlags & RF_IS_EDITOR ) ) {
	...
	for ( int i = 0 ; i < surfaces.Num() ; i++ ) {
		EmitSurface( &surfaces[i], viewDef->worldSpace.modelMatrix, viewDef->worldSpace.modelViewMatrix, 0, NULL );
	}
