...
void idRenderSystemLocal::GetValidModes( idStr &mode4x3Text, idStr &mode4x3Values,
		idStr &mode16x9Text, idStr &mode16x9Values,
		idStr &mode16x10Text, idStr &mode16x10Values ) {
	mode4x3Text = "640x480;800x600;1024x768;1280x960;1600x1200";
	mode4x3Values = "3;4;5;6;7";
	mode16x9Text = "1280x720;1366x768;1600x900;1920x1080";
	mode16x9Values = "-1;-1;-1;-1";
	mode16x10Text = "1280x800;1440x900;1680x1050;1920x1200";
	mode16x10Values = "-1;-1;-1;-1";
}

void idRenderSystemLocal::RemoveAllModelReferences( idRenderModel *model ) {
	for ( int i = 0; i < worlds.Num(); ++i ) {
		if ( worlds[i] ) {
			worlds[i]->RemoveAllModelReferences( model );
		}
	}
}
...
void idRenderSystemLocal::CopyPrimBatchTriangles( idDrawVert *destDrawVerts,
		glIndex_t *destIndices, rvMesh *primBatchMesh, const rvSilTraceVertT *silTraceVerts ) {
	if ( primBatchMesh ) {
		primBatchMesh->CopyTriangles( destDrawVerts, destIndices, silTraceVerts );
	}
}
...
void idRenderSystemLocal::DrawStretchCopy( float x, float y, float w, float h,
		float s1, float t1, float s2, float t2, const idMaterial *material ) {
...
void idRenderSystemLocal::DrawTinyChar( int x, int y, int ch, const idMaterial *material ) {
	...
	const float row = ( ch >> 4 ) * 0.0625f;
	const float col = ( ch & 15 ) * 0.0625f;
	DrawStretchPic( static_cast<float>( x ), static_cast<float>( y ), 4.0f, 8.0f,
		col, row, col + 0.0625f, row + 0.0625f, material );
}

void idRenderSystemLocal::DrawTinyStringExt( int x, int y, const char *string,
		const idVec4 &setColor, bool forceColor, const idMaterial *material ) {
	SetColor( setColor );
	for ( const char *s = string; s && *s; ++s ) {
		int escapeType = 0;
		const int escapeLength = idStr::IsEscape( s, &escapeType );
		if ( escapeLength ) {
			if ( !forceColor && escapeType == S_ESCAPE_COLORINDEX ) {
				SetColor( idStr::ColorForIndex( s[1] ) );
			}
			s += escapeLength - 1;
			continue;
		}
		DrawTinyChar( x, y, *s, material );
		x += 4;
	}
	SetColor( setColor );
}

void idRenderSystemLocal::BeginFrame( viewDef_t *newViewDef, int windowWidth, int windowHeight ) {
	viewDef = newViewDef;
...
void idRenderSystemLocal::GlobalToNormalizedDeviceCoordinates( const idVec3 &global, idVec3 &ndc ) {
	if ( tr.viewDef ) {
		R_GlobalToNormalizedDeviceCoordinates( tr.viewDef->worldSpace.modelViewMatrix, tr.viewDef->projectionMatrix, global, ndc );
	} else {
		R_GlobalToNormalizedDeviceCoordinates( tr.primaryView->worldSpace.modelViewMatrix, tr.primaryView->projectionMatrix, global, ndc );
	}
}
...
void idRenderSystemLocal::DrawSmallStringExt( int x, int y, const char *string, const idVec4 &setColor, bool forceColor, const idMaterial *material ) {
	...
	while ( *s ) {
		int escapeType = 0;
		const int escapeLength = idStr::IsEscape( (const char *)s, &escapeType );
		if ( escapeLength ) {
			if ( !forceColor && escapeType == S_ESCAPE_COLORINDEX ) {
			...
			} else if ( !forceColor && escapeType == S_ESCAPE_COLOR && escapeLength == 5 ) {
				color.Set( ( s[2] - '0' ) / 9.0f, ( s[3] - '0' ) / 9.0f, ( s[4] - '0' ) / 9.0f, setColor[3] );
				SetColor( color );
			}
			s += escapeLength;
			continue;
...
void idRenderSystemLocal::EndFrame( int *frontEndMsec, int *backEndMsec, int *numVerts, int *numIndexes ) {
	...
	if ( numVerts ) {
		*numVerts = backEnd.pc.c_drawElements;
	}
	if ( numIndexes ) {
...
void idRenderSystemLocal::CaptureRenderToMemory( void *buffer ) {
	if ( !glConfig.isInitialized || !buffer ) {
		return;
	}
	renderCrop_t *rc = &renderCrops[currentRenderCrop];
	guiModel->EmitFullScreen();
	...
	const int paddedPixels = ( rc->width + 3 ) * rc->height;
	byte *rgb = static_cast<byte *>( R_StaticAlloc( paddedPixels * 3 ) );
	qglReadPixels( rc->x, rc->y, rc->width, rc->height, GL_RGB, GL_UNSIGNED_BYTE, rgb );
	byte *rgba = static_cast<byte *>( buffer );
	for ( int i = 0; i < paddedPixels; ++i ) {
...
void idRenderSystemLocal::DebugGraph( float cur, float minValue, float maxValue, const idVec4 &color ) {
	for ( int y = 0; y < 64; ++y ) {
		memmove( &debugGraph[y][0][0], &debugGraph[y][1][0], 255 * 4 );
	}
	const float range = maxValue - minValue;
	const float normalized = range != 0.0f ? ( maxValue - cur ) / range : 0.0f;
	const int split = idMath::ClampInt( 0, 64, idMath::Ftoi( normalized * 64.0f ) );
	for ( int y = 0; y < 64; ++y ) {
		byte *pixel = debugGraph[y][255];
		if ( y < split ) {
			memset( pixel, 0, 4 );
		} else {
			pixel[0] = static_cast<byte>( idMath::ClampInt( 0, 255, idMath::Ftoi( color.x * 255.0f ) ) );
			pixel[1] = static_cast<byte>( idMath::ClampInt( 0, 255, idMath::Ftoi( color.y * 255.0f ) ) );
			pixel[2] = static_cast<byte>( idMath::ClampInt( 0, 255, idMath::Ftoi( color.z * 255.0f ) ) );
			pixel[3] = static_cast<byte>( idMath::ClampInt( 0, 255, idMath::Ftoi( color.w * 255.0f ) ) );
		}
	}
}

void idRenderSystemLocal::ShowDebugGraph() {
	if ( !debugGraphMaterial ) {
		return;
	}
	UploadImage( "textures/common/debuggraph.tga", &debugGraph[0][0][0], 256, 64 );
	SetColor4( 1, 1, 1, 1 );
	DrawStretchPic( 10, 400, 256, 64, 0, 0, 1, 1, debugGraphMaterial );
}
...
==============
*/
void idRenderSystemLocal::PrintMemInfo( MemInfo *mi ) {
	// sum up image totals
