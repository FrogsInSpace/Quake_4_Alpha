...
===========
*/
vertCache_t	*idVertexCache::AllocFrameTemp( void *data, int size, unsigned int flags ) {
	vertCache_t	*block;
	...
	if ( dynamicAllocThisFrame + size > frameBytes ) {
		...
		tempOverflow = true;
		Alloc( data, size, &block, ( flags & 1 ) != 0 );
		Free( block);
		return block;
	}
	...
	if ( block->vbo ) {
		const GLenum target = block->indexBuffer ? GL_ELEMENT_ARRAY_BUFFER_ARB : GL_ARRAY_BUFFER_ARB;
		qglBindBufferARB( target, block->vbo );
		qglBufferSubDataARB( target, block->offset, (GLsizeiptrARB)size, data );
	} else {
