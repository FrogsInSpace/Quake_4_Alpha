...
int idCollisionModelManagerLocal::ContentsFromString( const char *string ) const {
	int contents = 0;
	idLexer lexer( string != NULL ? string : "", string != NULL ? idStr::Length( string ) : 0, "ContentsFromString" );
	idToken token;
	while ( lexer.ReadToken( &token ) ) {
		if ( token == "," ) continue;
		for ( int i = 1; cm_contentsNameByIndex[i] != NULL; ++i ) if ( !token.Icmp( cm_contentsNameByIndex[i] ) ) { contents |= cm_contentsFlagByIndex[i]; break; }
	}
	return contents;
}

const char *idCollisionModelManagerLocal::StringFromContents( int contents ) const {
	static char contentsString[MAX_STRING_CHARS];
	int length = 0;
	contentsString[0] = '\0';
	for ( int i = 1; cm_contentsFlagByIndex[i] != 0; ++i ) if ( contents & cm_contentsFlagByIndex[i] ) {
		if ( length > 0 ) length += idStr::snPrintf( contentsString + length, sizeof( contentsString ) - length, "," );
		length += idStr::snPrintf( contentsString + length, sizeof( contentsString ) - length, "%s", cm_contentsNameByIndex[i] );
	}
	return contentsString;
}

void idCollisionModelManagerLocal::DrawEdge( idCollisionModelLocal *model, int edgeNum,
		const idVec3 &modelOrigin, const idMat3 &modelAxis, const idVec3 &viewOrigin,
		const idMat3 &viewAxis, float radius ) {}
void idCollisionModelManagerLocal::DrawPolygon( idCollisionModelLocal *model, cm_polygon_t *polygon,
		const idVec3 &modelOrigin, const idMat3 &modelAxis, const idVec3 &viewOrigin,
		const idMat3 &viewAxis, float radius ) {}
...
void idCollisionModelManagerLocal::SpeedTest( const idVec3 &viewOrigin ) {}

void idCollisionModelManagerLocal::DrawModel( idCollisionModel *model, const idVec3 &modelOrigin,
		const idMat3 &modelAxis, const idVec3 &viewOrigin, const idMat3 &viewAxis, float radius ) {
	if ( model == NULL ) return;
	idCollisionModelLocal *localModel = static_cast<idCollisionModelLocal *>( model );
	DrawNodePolygons( localModel, localModel->node, modelOrigin, modelAxis, viewOrigin, viewAxis, radius );
}

void idCollisionModelManagerLocal::DebugOutput( const idVec3 &viewOrigin, const idMat3 &viewAxis ) {
	if ( cm_drawMask.IsModified() ) cm_drawMask.ClearModified();
}
