...
void idCollisionModelManagerLocal::WriteCollisionModelsToFile( const char *filename, unsigned int mapFileCRC ) {
	// Retail ownership and signature are restored here.  Serialization remains
	// disabled until the Q4 array-based model format is fully validated.
}

bool idCollisionModelManagerLocal::WriteCollisionModelForMapEntity( const idMapEntity *mapEnt,
		const char *filename, bool testTraceModel ) {
	return false;
}
