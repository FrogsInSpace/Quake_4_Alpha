...
typedef struct cm_polygon_s {
	...
	idVec2					texBounds[3];		// texture-coordinate bounds for material type lookup
	int						checkcount;			// for multi-check avoidance
	...
	int *					edges;				// indexes into the model polygon-edge array
} cm_polygon_t;
...
typedef struct cm_brush_s {
	...
	idPlane *				planes;				// planes in the model brush-plane array
} cm_brush_t;
...
#if defined( _WIN32 )
static_assert( sizeof( cm_vertex_t ) == 0x18, "cm_vertex_t ABI drift" );
static_assert( sizeof( cm_edge_t ) == 0x24, "cm_edge_t ABI drift" );
static_assert( sizeof( cm_polygon_t ) == 0x58, "cm_polygon_t ABI drift" );
static_assert( sizeof( cm_brush_t ) == 0x30, "cm_brush_t ABI drift" );
static_assert( sizeof( cm_node_t ) == 0x1C, "cm_node_t ABI drift" );
#endif
...
===============================================================================
	...
	Quake 4 collision model object

	The retail executable replaced Doom 3's integer cmHandle_t API with an
	idCollisionModel object.  This layout is taken from the matching retail PDB
	(idCollisionModelLocal, size 0xC8 on Win32).  The older cm_model_t above is
	kept temporarily while the seeded Doom 3 manager implementation is ported
...
public:
	virtual				~idCollisionModelLocal( void );
...
#if defined( _WIN32 )
static_assert( sizeof( idCollisionModelLocal ) == 0xC8, "idCollisionModelLocal ABI drift" );
#endif
...
// Retail Quake 4 layouts.  These deliberately differ from Doom 3's trace
// workspace above; member order and padding come from the matching PDB.
typedef struct cm_trmVertex_t {
	idVec3		p;
	int			used;
	idVec3		endp;
	int			pad0;
...
typedef struct cm_trmPolygon_t {
	idPlane		plane;
	int			used;
	int			numEdges;
...
#if defined( _WIN32 )
static_assert( sizeof( cm_trmVertex_t ) == 0x60, "cm_trmVertex_t ABI drift" );
static_assert( sizeof( cm_trmEdge_t ) == 0x60, "cm_trmEdge_t ABI drift" );
static_assert( sizeof( cm_trmPolygon_t ) == 0x70, "cm_trmPolygon_t ABI drift" );
static_assert( offsetof( cm_traceWork_t, numVerts ) == 0x1F60, "cm_traceWork_t::numVerts ABI drift" );
static_assert( offsetof( cm_traceWork_t, model ) == 0x1FD0, "cm_traceWork_t::model ABI drift" );
static_assert( offsetof( cm_traceWork_t, trace ) == 0x2094, "cm_traceWork_t::trace ABI drift" );
static_assert( offsetof( cm_traceWork_t, polygonEdgePlueckerCache ) == 0x2114, "cm_traceWork_t cache ABI drift" );
static_assert( sizeof( cm_traceWork_t ) == 0x2D14, "cm_traceWork_t ABI drift" );
#endif
...
#ifndef Q4_CM_MODEL_ONLY

// Compatibility surface for compiling the Doom 3 seed while each manager
// translation unit is converted to Quake 4's object-based public API.
#if defined( Q4_CM_LEGACY_SEED )
...
public:
	...
	virtual void		Translation( trace_t *results, const idVec3 &start, const idVec3 &end, const idTraceModel *trm, const idMat3 &trmAxis, int contentMask, idCollisionModel *model, const idVec3 &modelOrigin, const idMat3 &modelAxis );
	virtual void		Rotation( trace_t *results, const idVec3 &start, const idRotation &rotation, const idTraceModel *trm, const idMat3 &trmAxis, int contentMask, idCollisionModel *model, const idVec3 &modelOrigin, const idMat3 &modelAxis );
	virtual int		Contents( const idVec3 &start, const idTraceModel *trm, const idMat3 &trmAxis, int contentMask, idCollisionModel *model, const idVec3 &modelOrigin, const idMat3 &modelAxis );
	virtual int		Contacts( contactInfo_t *contacts, const int maxContacts, const idVec3 &start, const idVec6 &dir, const float depth, const idTraceModel *trm, const idMat3 &trmAxis, int contentMask, idCollisionModel *model, const idVec3 &modelOrigin, const idMat3 &modelAxis );
	...
	void				PolygonFromWinding( idCollisionModelLocal *model, idFixedWinding *winding, const idPlane &plane, const idMaterial *material, int primitiveNum );

private:
	void				Clear( void );
	void				ClearModel( idCollisionModelLocal *model );
	...
	void				CopyPolygon( cm_polygon_t *dest, cm_polygon_t *src );
	...
	cm_node_t *		PointNode( const idVec3 &point, idCollisionModelLocal *model );
	int				PointContents( const idVec3 point, idCollisionModelLocal *model );
	int				TransformedPointContents( const idVec3 &point, idCollisionModelLocal *model, const idVec3 &origin, const idMat3 &modelAxis );
	int				ContentsTrm( trace_t *results, const idVec3 &start, const idTraceModel *trm, const idMat3 &trmAxis,
						int contentMask, idCollisionModel *model, const idVec3 &modelOrigin, const idMat3 &modelAxis );
	void				SetupTrm( cm_traceWork_t *traceWork, const idTraceModel *trm );
	void				TraceThroughModel( cm_traceWork_t *traceWork );
	void				TraceThroughAxialBSPTree_r( cm_traceWork_t *traceWork, cm_node_t *node,
						float p1f, float p2f, idVec3 &p1, idVec3 &p2 );
	void				TraceTrmThroughNode( cm_traceWork_t *traceWork, cm_node_t *node );
	bool				TestTrmVertsInBrush( cm_traceWork_t *traceWork, cm_brush_t *brush );
	...
	const char *		GetFullModelName( const char *mapName, const char *modelName );
	idCollisionModelLocal *FindModel( const char *name );

	// CollisionModel_rotate.obj
	...
	int				RotatePointThroughEpsilonPlane( const cm_traceWork_t *tw, const idVec3 &point, const idVec3 &endPoint,
						const idPlane &plane, float angle, const idVec3 &origin,
						float &tanHalfAngle, idVec3 &collisionPoint, idVec3 &endDir );
	void			RotateTrmVertexThroughPolygon( cm_traceWork_t *tw, cm_polygon_t *poly, cm_trmVertex_t *vertex, int vertexNum );
	void			RotateVertexThroughTrmPolygon( cm_traceWork_t *tw, cm_trmPolygon_t *trmPoly, cm_polygon_t *poly,
						cm_vertex_t *vertex, idVec3 &rotationOrigin );
	void			Rotation180( trace_t *results, const idVec3 &rotationOrigin, const idVec3 &axis,
						float startAngle, float endAngle, const idVec3 &start, const idTraceModel *trm,
						const idMat3 &trmAxis, int contentMask, idCollisionModel *model,
						const idVec3 &modelOrigin, const idMat3 &modelAxis );

	// CollisionModel_files.obj
	void			WriteNodes( idFile *file, cm_node_t *node );
	cm_node_t *		ParseNodes( Lexer *lexer, idCollisionModelLocal *model, cm_node_t *parent );
	void			WritePolygons( idFile *file, cm_node_t *node );
	void			WriteBrushes( idFile *file, cm_node_t *node );
	void			ParseVertices( Lexer *lexer, idCollisionModelLocal *model );
	void			ParseEdges( Lexer *lexer, idCollisionModelLocal *model );
	void			ParsePolygons( Lexer *lexer, idCollisionModelLocal *model );
	void			ParseBrushes( Lexer *lexer, idCollisionModelLocal *model );
	void			WriteCollisionModel( idFile *file, idCollisionModelLocal *model );
	void			WriteCollisionModelsToFile( const char *filename, unsigned int mapFileCRC );
	bool			ParseCollisionModel( Lexer *lexer, const char *filename, unsigned int mapFileCRC );
	bool			LoadCollisionModelFile( const char *filename, unsigned int mapFileCRC );

	// CollisionModel_debug.obj
	const char *	StringFromContents( int contents ) const;
	int				ContentsFromString( const char *string ) const;
	void			DrawEdge( idCollisionModelLocal *model, int edgeNum, const idVec3 &modelOrigin, const idMat3 &modelAxis,
						const idVec3 &viewOrigin, const idMat3 &viewAxis, float radius );
	void			DrawPolygon( idCollisionModelLocal *model, cm_polygon_t *polygon, const idVec3 &modelOrigin, const idMat3 &modelAxis,
						const idVec3 &viewOrigin, const idMat3 &viewAxis, float radius );
	void			DrawNodePolygons( idCollisionModelLocal *model, cm_node_t *node, const idVec3 &modelOrigin, const idMat3 &modelAxis,
						const idVec3 &viewOrigin, const idMat3 &viewAxis, float radius );
	...
	void			SpeedTest( const idVec3 &viewOrigin );

private:
	idStr				mapName;
	unsigned int		mapFileTime;
	int					loaded;
	idList<idCollisionModelLocal *> models;
	int					checkCount;
	bool				getContacts;
	contactInfo_t *	contacts;
	int					maxContacts;
...
#if defined( _WIN32 )
static_assert( sizeof( idCollisionModelManagerLocal ) == 0x88, "idCollisionModelManagerLocal ABI drift" );
#endif
...
public:
#if defined( Q4_CM_LEGACY_SEED )
	// Quake 4 virtual surface.  Implementations replace the legacy overloads
	// incrementally; declaring them here also keeps the seed class concrete.
	virtual void		Init( void );
	virtual void		Shutdown( void );
	virtual void		LoadMap( const idMapFile *mapFile, bool forceReload );
	virtual void		FreeMap( const char *mapName );
