...
int idCollisionModelManagerLocal::CollisionBetweenEdgeBounds( cm_traceWork_t *tw,
		const idVec3 &va, const idVec3 &vb, const idVec3 &vc, const idVec3 &vd,
		float tanHalfAngle, idVec3 &collisionPoint, idVec3 &collisionNormal ) {
	return 0;
}
...
int idCollisionModelManagerLocal::RotatePointThroughEpsilonPlane( const cm_traceWork_t *tw,
		const idVec3 &point, const idVec3 &endPoint, const idPlane &plane, float angle,
		const idVec3 &origin, float &tanHalfAngle, idVec3 &collisionPoint, idVec3 &endDir ) {
	return 0;
}

void idCollisionModelManagerLocal::RotateTrmVertexThroughPolygon( cm_traceWork_t *tw, cm_polygon_t *poly, cm_trmVertex_t *vertex, int vertexNum ) {}
void idCollisionModelManagerLocal::RotateVertexThroughTrmPolygon( cm_traceWork_t *tw, cm_trmPolygon_t *trmPoly, cm_polygon_t *poly, cm_vertex_t *vertex, idVec3 &rotationOrigin ) {}
bool idCollisionModelManagerLocal::RotateTrmThroughPolygon( cm_traceWork_t *tw, cm_polygon_t *polygon ) { return false; }

bool CM_RayIntersectsConvexTraceModel( const cm_traceWork_t *tw, const idVec3 &start, const idVec3 &dir ) {
	return false;
}

void idCollisionModelManagerLocal::Rotation180( trace_t *results, const idVec3 &rotationOrigin,
		const idVec3 &axis, float startAngle, float endAngle, const idVec3 &start,
		const idTraceModel *trm, const idMat3 &trmAxis, int contentMask,
		idCollisionModel *model, const idVec3 &modelOrigin, const idMat3 &modelAxis ) {
	idRotation rotation( rotationOrigin, axis, endAngle - startAngle );
	Rotation( results, start, rotation, trm, trmAxis, contentMask, model, modelOrigin, modelAxis );
}

void idCollisionModelManagerLocal::Rotation( trace_t *results, const idVec3 &start,
	...
	results->endpos = rotation * start;
	results->endAxis = trmAxis * rotation.ToMat3();

	// Preserve solid-start behavior until the rotational edge solver is fully
	// transcribed: position contents are exact and prevent tunnelling out of a
	// model when a rotation begins embedded.
	if ( model != NULL && Contents( start, trm, trmAxis, contentMask, model, modelOrigin, modelAxis ) & contentMask ) {
