...
void idSimpleWindow::ResetCinematics() {
	if ( background ) {
...
idWinVar *idSimpleWindow::GetWinVarByName(const char *_name) {
	...
	if ( underscore && underscore[1] && !underscore[2] ) {
		...
		if ( component ) {
			...
			idWinVar *parentVar = GetWinVarByName( baseName );
...
bool idSimpleWindow::GetMaxTextIndex( const char *text, wrapInfo_t& wrapInfo ) const {
	if ( !dc ) {
		return false;
	}
	dc->SetFont( fontNum );
