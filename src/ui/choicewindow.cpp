...
void idChoiceWindow::ValidateChoice() {
	...
	if ( choices.Num() == 0 ) {
		choices.Append( common->GetLocalizedString( "#str_107731" ) );
	}
}
			...
			while( src.ReadToken( &token ) ) {
				if ( token == ";" ) {
					if ( str2.Length() ) {
						...
						str2 = common->GetLocalizedString( str2 );
						choices.Append(str2);
						str2 = "";
					}
					continue;
...
void idChoiceWindow::Draw(int time, float x, float y) {
	...
	// The retail Q4 implementation leaves an empty choice window undrawn.  A
	// dynamic GUI expression can temporarily clear choices after PostParse.
	if ( choices.Num() == 0 ) {
		return;
	}
