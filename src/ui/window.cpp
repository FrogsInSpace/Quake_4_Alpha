		...
		if ( data->interp.IsDone( gui->GetTime() ) && data->data) {
			if (v4) {
				...
				*val = data->interp.GetEndValue()[0];
				...
				member->Set( data->interp.GetEndValue()[0] );
			} else {
			...
			if (data->data) {
				if (v4) {
					...
					*val = data->interp.GetCurrentValue( gui->GetTime() )[0];
					...
					member->Set( data->interp.GetCurrentValue( gui->GetTime() )[0] );
				} else {
...
idWinVar *idWindow::GetWinVarByName(const char *_name, bool fixup, drawWin_t** owner) {
	...
	// Quake 4 GUI scripts may address a vec4/rectangle component by suffix.
	// Keep these proxy variables in the window's defined-var list so repeated
	// lookups return the same object and transitions retain a stable target.
	const char *underscore = strrchr( _name, '_' );
	if ( underscore && underscore[1] && !underscore[2] ) {
		const char *component = strchr( "xyzw", underscore[1] );
		if ( component ) {
			idStr baseName( _name, 0, static_cast<int>( underscore - _name ) );
			idWinVar *parentVar = GetWinVarByName( baseName, fixup, owner );
			if ( dynamic_cast<idWinVec4 *>( parentVar ) || dynamic_cast<idWinRectangle *>( parentVar ) ) {
				idWinFloatMember *member = new idWinFloatMember( parentVar, static_cast<int>( component - "xyzw" ) );
...
//  added new onEvent
			...
			if ( token == "+" ) {
				...
				ev->time = timeLineEvents.Num() ? timeLineEvents[timeLineEvents.Num() - 1]->time : 0;
			...
			idToken iconName;
			idToken iconMaterial;
			if ( !src->ReadToken( &iconName ) || !src->ReadToken( &iconMaterial ) ) {
				src->Error( "Unexpected end of file in defineicon" );
				ret = false;
				break;
			}
			...
			if ( src->ReadToken( &optional ) ) {
				if ( optional == "," ) {
					...
					src->UnreadToken( &optional );
				}
			}
			dc->RegisterIcon( iconName, iconMaterial, iconX, iconY, iconW, iconH );
		}
...
bool idWindow::GetMaxTextIndex( const char *text, wrapInfo_t& wrapInfo ) const {
	if ( !dc ) {
		return false;
	}
	dc->SetFont( fontNum );
	return dc->GetMaxTextIndex( text, static_cast<int>( textRect.w ), textScale, wrapInfo );
}
