...
public:
	...
	virtual void SetMember( int index, const float val ) {}
	virtual float GetMember( int index ) const { return 0.0f; }
...
public:
	...
	int LengthWithoutColors() {
		...
		return data.LengthWithoutEscapes();
	}
	int Length() {
		if (guiDict && name && *name) {
			data = guiDict->GetString(GetName());
...
public:
	...
	virtual void SetMember( int index, const float val ) {
		...
		idVec4 value = data.ToVec4();
		value[index] = val;
		*this = value;
	}
	virtual float GetMember( int index ) const {
		if ( index < 0 || index > 3 ) {
			return 0.0f;
		}
		const idVec4 value = data.ToVec4();
		return value[index];
	}
	virtual void Update() {
		const char *s = GetName();
...
public:
	...
	virtual float GetMember( int index ) const {
		return ( index >= 0 && index < 4 ) ? data[index] : 0.0f;
	}
	virtual void Update() {
		const char *s = GetName();
		if ( guiDict && s[0] != '\0' ) {
...
// Quake 4 exposes vector and rectangle components to GUI scripts through
// synthetic variables such as matColor_w and rect_x.
class idWinFloatMember : public idWinVar {
public:
	idWinFloatMember( idWinVar *parent = NULL, int member = 0 ) :
		idWinVar(), parentVar( parent ), memberIndex( member ) {}
	virtual ~idWinFloatMember() {}

	virtual void Init( const char *_name, idWindow *win ) {
		idWinVar::Init( _name, win );
	}
	virtual void Set( const char *val ) {
		Set( static_cast<float>( atof( val ) ) );
	}
	void Set( const float val ) {
		if ( parentVar ) {
			parentVar->SetMember( memberIndex, val );
		}
	}
	virtual void Update() {
		if ( parentVar ) {
	...
	virtual void WriteToSaveGame( idFile *savefile ) {
		const float value = x();
		savefile->Write( &eval, sizeof( eval ) );
		savefile->Write( &value, sizeof( value ) );
	}
	virtual void ReadFromSaveGame( idFile *savefile ) {
		float value;
		savefile->Read( &eval, sizeof( eval ) );
		savefile->Read( &value, sizeof( value ) );
		Set( value );
	}
	virtual float x() const {
		return parentVar ? parentVar->GetMember( memberIndex ) : 0.0f;
	}

	idWinVar *GetParentVar() const { return parentVar; }
