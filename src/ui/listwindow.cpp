...
bool idListWindow::ParseInternalVar(const char *_name, idParser *src) {
	...
	if(idStr::Icmp(_name, "tabTextScales") == 0) {
		ParseString(src, tabTextScalesStr);
		return true;
	}
...
void idListWindow::PostParse() {
	...
	if (tabTextScalesStr.Length()) {
		idParser src(tabTextScalesStr, tabTextScalesStr.Length(), "tabtextscales", LEXFL_NOFATALERRORS | LEXFL_NOSTRINGCONCAT | LEXFL_NOSTRINGESCAPECHARS);
		idToken tok;
		while (src.ReadToken(&tok)) {
			if (tok == ",") {
				continue;
		...
		if ( tabInfo.Num() > 0 ) {
			...
			while ( start < listItems[i].Length() ) {
				...
				if ( tabInfo[tab].type == TAB_TYPE_TEXT ) {
					dc->DrawText(work, tabInfo[tab].textScale, tabInfo[tab].align, color, rect, false, -1);
