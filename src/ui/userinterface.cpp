...
void idUserInterfaceManagerLocal::EndLevelLoad() {
	...
	for ( int i = 0; i < alwaysThinkGUIs.Num(); i++ ) {
		if ( guis.FindIndex( alwaysThinkGUIs[i] ) < 0 ) {
	...
	if ( gui ) {
		...
		for ( int i = 0; i < c; i++ ) {
			if ( guis[i] == gui ) {
				delete guis[i];
				guis.RemoveIndex( i );
				alwaysThinkGUIs.Remove( static_cast<idUserInterfaceLocal *>( gui ) );
				return;
			}
		}
	}
...
void idUserInterfaceManagerLocal::RunAlwaysThinkGUIs( int time ) {
	for ( int i = 0; i < alwaysThinkGUIs.Num(); i++ ) {
		...
		gui->SetTime( time );
		gui->GetDesktop()->RunTimeEvents( time );
	}
}
...
bool idUserInterfaceLocal::InitFromFile( const char *qpath, bool rebuild, bool cache ) { 
	...
	if ( ( desktop->GetFlags() & WIN_ALWAYSTHINK ) != 0 && uiManagerLocal.alwaysThinkGUIs.Find( this ) == NULL ) {
		uiManagerLocal.alwaysThinkGUIs.Append( this );
	}
...
void idUserInterfaceLocal::ClearState() {
	state.Clear();
}
...
idVec4 idUserInterfaceLocal::GetLightColor() {
	return lightColorVar ? static_cast<const idVec4&>( *lightColorVar ) : vec4_origin;
}
...
bool idUserInterfaceLocal::GetMaxTextIndex( const char *windowName, const char *text, wrapInfo_t& wrapInfo ) const {
	...
	drawWin_t *drawWin = desktop->FindChildByName( windowName );
	if ( !drawWin ) {
		return false;
	}
	if ( drawWin->win ) {
		return drawWin->win->GetMaxTextIndex( text, wrapInfo );
	}
	return drawWin->simp ? drawWin->simp->GetMaxTextIndex( text, wrapInfo ) : false;
}
