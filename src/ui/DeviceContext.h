...
public:
	...
	bool				GetMaxTextIndex( const char *text, int width, float scale, wrapInfo_t& wrapInfo );
	...
	void				RegisterIcon( const char *code, const char *shader, int x, int y, int w, int h );
	void				SizeIcons( void );
...
private:
	int					DrawText(float x, float y, float scale, idVec4 color, const char *text, float adjust, int limit, int style, int cursor = -1, bool resetEscapes = true);
	...
	void				Clear( void );
	void				SizeIcon( embeddedIcon_s *icon );
	...
	idHashTable<embeddedIcon_s> icons;
};

#if defined( _WIN32 ) && defined( Q4_RECON_ENGINE_PRIVATE )
static_assert( sizeof( idDeviceContext ) == 0xF0, "idDeviceContext ABI drift" );
#endif
