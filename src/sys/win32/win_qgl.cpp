...
// WGL_ARB_pbuffer / WGL_ARB_render_texture data symbols owned by win_qgl.obj.
...
static const char *FloatData( const GLfloat *v, int count ) {
	...
	fprintf( qglLogFile, "static float %s[%i] = {", name, count );
	for( int i = 0 ; i < count ; i++ ) {
		if ( i < count - 1 ) {
			fprintf( qglLogFile, "%f,", v[i] );
		} else {
			fprintf( qglLogFile, "%f};\n", v[i] );
		}
	}
...
{
	fprintf( qglLogFile, "glClear( 0 " );

	if ( mask & GL_COLOR_BUFFER_BIT )
		fprintf( qglLogFile, "| GL_COLOR_BUFFER_BIT " );
	if ( mask & GL_DEPTH_BUFFER_BIT )
		fprintf( qglLogFile, "| GL_DEPTH_BUFFER_BIT " );
	if ( mask & GL_STENCIL_BUFFER_BIT )
		fprintf( qglLogFile, "| GL_STENCIL_BUFFER_BIT " );
	if ( mask & GL_ACCUM_BUFFER_BIT )
		fprintf( qglLogFile, "| GL_ACCUM_BUFFER_BIT " );
...
#if !defined( Q4_DISABLE_GL_LOGGING )
...
void GLimp_EnableLogging( bool enable ) {
	...
	if ( enable ) {
		if ( !qglLogFile ) {
			...
			qglLogFile = fopen( ospath, "wt" );

			// write the time out to the top of the file
			time( &aclock );
			newtime = localtime( &aclock );
			fprintf( qglLogFile, "// %s", asctime( newtime ) );
			fprintf( qglLogFile, "// %s\n\n", cvarSystem->GetCVarString( "si_version" ) );
		}
