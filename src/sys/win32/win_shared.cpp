...
// idMath deliberately undefines the CRT integer limit macros. ATL's legacy
// allocation helpers still consume them as macros.
#ifndef INT_MIN
...
int Sys_MapKey( unsigned long key, unsigned short wParam ) {
	const unsigned int scanCode = ( key >> 16 ) & 0xff;
	if ( scanCode == 0x29 ) {
		return 1;
	}
	if ( scanCode == 0x2a ) {
	...
	const int mapped = MapKey( (int)key );
	if ( mapped ) {
		return mapped;
	}
	return (int)MapVirtualKeyA( wParam, MAPVK_VK_TO_CHAR );
}
