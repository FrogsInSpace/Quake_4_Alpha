...
public:
	...
	idBlockAlloc<udpMsg_t, 64, MA_NETWORK> udpMsgAllocator;
};
...
bool idPort::InitForPort( int portNumber ) {
	netSocket = NET_IPSocket( net_ip.GetString(), portNumber );
	if ( netSocket <= 0 ) {
		netSocket = 0;
		port = 0;
		return false;
	...
	if ( portNumber == PORT_ANY ) {
		...
		getsockname( netSocket, (sockaddr *)&address, &len );
...
bool idPort::GetPacket( netadr_t &from, void *data, int &size, int maxSize ) {
	...
	while( 1 ) {
		...
		if ( net_forceLatency.GetInteger() > 0 ) {
			...
			msg = udpPorts[ port ]->udpMsgAllocator.Alloc();
			memcpy( msg->data, data, size );
			msg->size = size;
			msg->address = from;
			msg->time = Sys_Milliseconds();
			msg->next = NULL;
			if ( udpPorts[ port ]->recieveLast ) {
				udpPorts[ port ]->recieveLast->next = msg;
			} else {
				udpPorts[ port ]->recieveFirst = msg;
			}
			udpPorts[ port ]->recieveLast = msg;
		} else {
			break;
		}
	}

	if ( net_forceLatency.GetInteger() > 0 || ( udpPorts[ port ] && udpPorts[ port ]->recieveFirst ) ) {

		msg = udpPorts[ port ]->recieveFirst;
		if ( msg && msg->time <= Sys_Milliseconds() - net_forceLatency.GetInteger() ) {
			memcpy( data, msg->data, msg->size );
			size = msg->size;
			from = msg->address;
			udpPorts[ port ]->recieveFirst = udpPorts[ port ]->recieveFirst->next;
			if ( !udpPorts[ port ]->recieveFirst ) {
				udpPorts[ port ]->recieveLast = NULL;
			}
			udpPorts[ port ]->udpMsgAllocator.Free( msg );
			return true;
		}
		return false;
...
void idPort::SendPacket( const netadr_t to, const void *data, int size ) {
	...
	if ( net_forceLatency.GetInteger() > 0 || ( udpPorts[ port ] && udpPorts[ port ]->sendFirst ) ) {
		...
		msg->time = Sys_Milliseconds();
		msg->next = NULL;
		if ( udpPorts[ port ]->sendLast ) {
			udpPorts[ port ]->sendLast->next = msg;
		} else {
			udpPorts[ port ]->sendFirst = msg;
		}
		udpPorts[ port ]->sendLast = msg;

		for ( msg = udpPorts[ port ]->sendFirst; msg && msg->time <= Sys_Milliseconds() - net_forceLatency.GetInteger(); msg = udpPorts[ port ]->sendFirst ) {
			Net_SendUDPPacket( netSocket, msg->size, msg->data, msg->address );
			udpPorts[ port ]->sendFirst = udpPorts[ port ]->sendFirst->next;
			if ( !udpPorts[ port ]->sendFirst ) {
				udpPorts[ port ]->sendLast = NULL;
			}
			udpPorts[ port ]->udpMsgAllocator.Free( msg );
		}
