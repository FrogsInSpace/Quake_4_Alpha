...
public:
	...
	void		Init( int spawnType = SPF_NONE_0 ) { mSpawnType = spawnType; mFlags = 0; mRange = 0.0f; mMisc = NULL; mMins.Zero(); mMaxs.Zero(); }
...
private:
	int			mSpawnType;				// Type of spawn domain (eg. box, spiral)
	int			mFlags;					// PPFLAG_
	float		mRange;					// Repeat length of spiral
	void		*mMisc;					// Domain-specific auxiliary data.
