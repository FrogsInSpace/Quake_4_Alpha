...
typedef enum eBSESegment
{
	...
	SEG_DOUBLEVISION,				// Triggers the double vision effect
...
public:
	...
	const		rvDeclEffect			*GetEffectDecl( void ) { return( mEffectDecl ); }
				...
				void					PlayEffect( rvBSE *effect, rvSegmentTemplate *st );
				...
				bool					Check( rvBSE *effect, float time );
				...
				void					UpdateElectricityParticles( float time );
				bool					UpdateParticles( rvBSE *effect, float time );
				float					EvaluateCost( void );
				...
				s_channelType			GetSoundChannel( void ) { return static_cast< s_channelType >( mSegmentTemplateHandle ) + SCHANNEL_ONE; }
				...
				void					InsertParticle( rvParticle *particle, rvSegmentTemplate *st );
				...
				void					RenderMotion( rvBSE *effect, const struct renderEffect_s *owner, rvRenderModelBSE *model, rvParticleTemplate *pt, float time );
				void					RenderTrail( rvBSE *effect, const struct renderEffect_s *owner, rvRenderModelBSE *model, float time );
				void					Render( rvBSE *effect, const struct renderEffect_s *owner, rvRenderModelBSE *model, float time );
...
public:
				...
				rvSegmentTemplate		&operator = ( const rvSegmentTemplate &copy );
				...
				bool					GetSmoker( void );
				bool					Parse( rvDeclEffect *et, int segmentType, idLexer *lexer );
				...
				void					CreateParticleTemplate( rvDeclEffect *et, idLexer *lexer, int particleType );
...
public:
	...
	static	void					CacheFromDict( const idDict& dict );
			...
			int						AddSegment( rvSegmentTemplate &segment );
			...
			bool					CompareToEditorOriginal( void );
			void					Revert( void );
...
public:
			friend class			rvSegment;
				...
				void					Init( rvDeclEffect *declEffect, struct renderEffect_s *parms, float time );
				void					Destroy( void );
				...
				void					SetStopped( bool stopped ) { SetFlag( stopped, EFLAG_STOPPED ); }
				...
				idSoundEmitter			*GetReferenceSound( int index ) const;
				int						GetReferenceSoundHandle( void ) const { return mReferenceSoundHandle; }
				...
				bool					Service( renderEffect_t *parms, float time );
				void					UpdateSoundEmitter( rvSegmentTemplate *st, rvSegment *seg );

				void					DisplayDebugInfo( const struct renderEffect_s *parms, const struct viewDef_s *view, idBounds &bounds );
	 			idRenderModel			*Render( const struct renderEffect_s *owner, const struct viewDef_s *view );
				...
				const idBounds			&GetCurrentLocalBounds( void ) const { return( mCurrentLocalBounds ); }
				const idBounds			&GetCurrentWorldBounds( void ) const { return( mCurrentWorldBounds ); }
...
public:
	...
	virtual	bool					ServiceEffect( rvRenderEffectLocal *def, float time );
...
public:
	static		idBlockAlloc<rvBSE, 256, MA_RENDER>	effects;
