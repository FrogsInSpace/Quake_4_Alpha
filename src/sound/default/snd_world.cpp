...
idSoundEmitterLocal *idSoundSystemLocal::AllocLocalSoundEmitter( int worldId, int requestedIndex ) {
	int index = requestedIndex;
	if ( index < 1 ) {
		for ( int i = 1; i < emitters.Num(); ++i ) {
			if ( emitters[i] == NULL || emitters[i]->removeStatus == REMOVE_STATUS_SAMPLEFINISHED ) { index = i; break; }
		}
	}
	if ( index < 1 ) index = emitters.Num();
	while ( emitters.Num() <= index ) emitters.Append( NULL );
	if ( emitters[index] == NULL ) emitters[index] = new idSoundEmitterLocal;
	idSoundEmitterLocal *emitter = emitters[index];
	emitter->Clear();
	emitter->index = index;
	emitter->worldId = worldId;
	emitter->removeStatus = REMOVE_STATUS_ALIVE;
	return emitter;
}

int idSoundSystemLocal::AllocSoundEmitter( int worldId ) {
	if ( !isInitialized || worldId <= SOUNDWORLD_NONE || worldId >= SOUNDWORLD_MAX ) return -1;
	return AllocLocalSoundEmitter( worldId, -1 )->Handle();
}

idSoundEmitter *idSoundSystemLocal::EmitterForIndex( int worldId, int index ) {
	if ( index <= 0 || index >= emitters.Num() ) return NULL;
	idSoundEmitterLocal *emitter = emitters[index];
	return emitter != NULL && emitter->AttachedToWorld( worldId ) ? emitter : NULL;
}

void idSoundSystemLocal::ClearAllSoundEmitters() {
	for ( int i = 0; i < emitters.Num(); ++i ) delete emitters[i];
	emitters.Clear();
	memset( localSound, 0, sizeof( localSound ) );
}

void idSoundSystemLocal::StopAllSounds( int worldId ) {
	for ( int i = 1; i < emitters.Num(); ++i ) if ( emitters[i] != NULL && emitters[i]->AttachedToWorld( worldId ) ) emitters[i]->StopSound( SCHANNEL_ANY );
}

void idSoundSystemLocal::ResetListener() {
	lastListenerArea = listenerArea;
	listenerArea = -1;
	listenerAreaName.Clear();
}

void idSoundSystemLocal::PlaceListener( const idVec3 &origin, const idMat3 &axis, int newListenerId, int newGameTime, const idStr &areaName ) {
	listenerPos = origin;
	listenerAxis = axis;
	listenerPrivateId = newListenerId;
	gameMsec = newGameTime;
	game44kHz = MillisecondsToSamples( newGameTime );
	listenerAreaName = areaName;
	lastListenerArea = listenerArea;
	listenerArea = rw != NULL ? rw->PointInArea( origin ) : -1;
}

void idSoundSystemLocal::ForegroundUpdate() {
	if ( !isInitialized || muted ) return;
	for ( int i = 1; i < emitters.Num(); ++i ) {
		idSoundEmitterLocal *emitter = emitters[i];
		if ( emitter == NULL || emitter->removeStatus == REMOVE_STATUS_SAMPLEFINISHED ) continue;
		emitter->CheckForCompletion( CurrentSoundTime );
		if ( emitter->playing ) emitter->Spatialize( listenerPos, listenerArea, rw );
		if ( emitter->removeStatus == REMOVE_STATUS_WAITSAMPLEFINISHED && !emitter->playing ) emitter->removeStatus = REMOVE_STATUS_SAMPLEFINISHED;
	}
}

float idSoundSystemLocal::CurrentShakeAmplitudeForPosition( int worldId, int time, const idVec3 &listenerPosition ) {
	float value = 0.0f;
	for ( int i = 1; i < emitters.Num(); ++i ) if ( emitters[i] != NULL && emitters[i]->AttachedToWorld( worldId ) && emitters[i]->hasShakes ) value = Max( value, emitters[i]->CurrentAmplitude() );
	return value;
}

void idSoundSystemLocal::FadeSoundClasses( int worldId, int soundClass, float to, float over ) {
	if ( soundClass < 0 || soundClass >= SOUND_MAX_CLASSES ) return;
	idSoundFade &fade = soundClassFade[soundClass];
	const int start = GetCurrent44kHzTime();
	fade.fadeStartVolume = fade.FadeDbAt44kHz( start );
	fade.fadeStart44kHz = start;
	fade.fadeEnd44kHz = start + (int)( over * PRIMARYFREQ );
	fade.fadeEndVolume = to;
}

void idSoundSystemLocal::StartWritingDemo( int worldId, idDemoFile *demo ) { writeDemo = demo; }
void idSoundSystemLocal::StopWritingDemo( int worldId ) { writeDemo = NULL; }
...
void idSoundSystemLocal::OffsetSoundTime( int offset44kHz ) { CurrentSoundTime += offset44kHz; game44kHz += offset44kHz; }

void idSoundSystemLocal::AVIOpen( int worldId, const char *path, const char *name ) {
	aviDemoPath = path != NULL ? path : "";
	aviDemoName = name != NULL ? name : "";
	lastAVI44kHz = game44kHz;
}
void idSoundSystemLocal::AVIUpdate() {}
void idSoundSystemLocal::AVIClose( int worldId ) { aviDemoPath.Clear(); aviDemoName.Clear(); }

void idSoundSystemLocal::ListActiveSounds( int worldId ) {
	int count = 0;
	for ( int i = 1; i < emitters.Num(); ++i ) if ( emitters[i] != NULL && emitters[i]->AttachedToWorld( worldId ) && emitters[i]->CurrentlyPlaying() ) ++count;
	common->Printf( "%d active sounds in world %d\n", count, worldId );
}
