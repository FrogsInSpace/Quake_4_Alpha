...
idCVar idSoundSystemLocal::s_noSound( "s_noSound", "0", CVAR_SOUND | CVAR_BOOL | CVAR_NOCHEAT, "disable sound output" );
idCVar idSoundSystemLocal::s_maxSoundsPerShader( "s_maxSoundsPerShader", "0", CVAR_SOUND | CVAR_ARCHIVE | CVAR_INTEGER, "maximum simultaneous sounds for one shader" );
idCVar idSoundSystemLocal::s_volume( "s_volume_dB", "0", CVAR_SOUND | CVAR_ARCHIVE | CVAR_FLOAT, "volume in dB" );
idCVar idSoundSystemLocal::s_useOpenAL( "s_useOpenAL", "0", CVAR_SOUND | CVAR_ARCHIVE | CVAR_BOOL, "use OpenAL" );
idCVar idSoundSystemLocal::s_deviceName( "s_deviceName", "", CVAR_SOUND | CVAR_ARCHIVE, "OpenAL device name" );

idSoundSystemLocal soundSystemLocal;
idSoundSystem *soundSystem = &soundSystemLocal;

s_stats::s_stats() :
	rinuse( 0 ), runs( 1 ), timeinprocess( 0 ), missedWindow( 0 ),
	missedUpdateWindow( 0 ), activeSounds( 0 ), numPortalChecks( 0 ) {
}

idSoundSystemLocal::idSoundSystemLocal() :
	rw( NULL ), writeDemo( NULL ), listenerPrivateId( 0 ), lastListenerArea( -1 ),
	listenerArea( -1 ), listenerEnvironmentID( 0 ), gameMsec( 0 ), game44kHz( 0 ),
	lastAVI44kHz( 0 ), activeSoundWorld( SOUNDWORLD_GAME ), snd_audio_hw( NULL ),
	soundCache( NULL ), olddwCurrentWritePos( 0 ), buffers( 0 ), CurrentSoundTime( 0 ),
	nextWriteBlock( 0 ), finalMixBuffer( NULL ), isInitialized( false ), muted( false ),
	shutdown( false ), voiceCommInit( false ), graph( NULL ), openalDevice( NULL ),
	openalContext( NULL ), openalSourceCount( 0 ), alEAXSet( NULL ), alEAXGet( NULL ),
	alEAXSetBufferMode( NULL ), alEAXGetBufferMode( NULL ), efxloaded( false ), eaxVersion( 0 ) {
	listenerAxis.Identity();
	listenerPos.Zero();
	memset( fpa, 0, sizeof( fpa ) );
	memset( localSound, 0, sizeof( localSound ) );
	memset( realAccum, 0, sizeof( realAccum ) );
	memset( meterTops, 0, sizeof( meterTops ) );
	memset( meterTopsTime, 0, sizeof( meterTopsTime ) );
	memset( openalSources, 0, sizeof( openalSources ) );
	memset( efxSlotAvailable, 0, sizeof( efxSlotAvailable ) );
	memset( efxSlotToArea, 0xff, sizeof( efxSlotToArea ) );
	memset( efxSlotId, 0, sizeof( efxSlotId ) );
...
void idSoundSystemLocal::Init() {
	...
	finalMixBuffer = (float *)( ( (uintptr_t)realAccum + 15 ) & ~(uintptr_t)15 );
	soundCache = new idSoundCache;
	emitters.Append( NULL );
	for ( int world = SOUNDWORLD_GAME; world < SOUNDWORLD_MAX; ++world ) {
		localSound[world] = AllocLocalSoundEmitter( world, emitters.Num() );
	}
	isInitialized = true;
	shutdown = false;
	common->Printf( "sound system initialized (reconstructed Q4 software backend)\n" );
}
...
void idSoundSystemLocal::ClearBuffer() {
	if ( finalMixBuffer != NULL ) {
		memset( finalMixBuffer, 0, sizeof( float ) * 6 * MIXBUFFER_SAMPLES );
	}
}

bool idSoundSystemLocal::InitHW() { return !s_noSound.GetBool(); }
bool idSoundSystemLocal::ShutdownHW() { openalDevice = NULL; openalContext = NULL; openalSourceCount = 0; return true; }
void idSoundSystemLocal::Frame() {}

int idSoundSystemLocal::AsyncMix( int soundTime, float *mixBuffer ) {
	CurrentSoundTime = soundTime;
	if ( mixBuffer != NULL ) {
		memset( mixBuffer, 0, sizeof( float ) * 6 * MIXBUFFER_SAMPLES );
	}
	return soundTime;
}

int idSoundSystemLocal::AsyncUpdate( int time ) {
	CurrentSoundTime = MillisecondsToSamples( time );
	return CurrentSoundTime;
}

int idSoundSystemLocal::AsyncUpdateWrite( int time ) { return AsyncUpdate( time ); }
void idSoundSystemLocal::SetMute( bool mute ) { muted = mute; if ( mute ) ClearBuffer(); }

cinData_t idSoundSystemLocal::ImageForTime( int milliseconds, bool waveform ) {
	cinData_t data;
...
int idSoundSystemLocal::GetSoundDecoderInfo( int index, soundDecoderInfo_t &decoderInfo ) {
	memset( &decoderInfo, 0, sizeof( decoderInfo ) );
	return 0;
}

void idSoundSystemLocal::BeginLevelLoad( const char *mapName ) { if ( soundCache ) soundCache->BeginLevelLoad( mapName ); }
void idSoundSystemLocal::EndLevelLoad( const char *mapName ) { if ( soundCache ) soundCache->EndLevelLoad( mapName ); }
void idSoundSystemLocal::PrintMemInfo( MemInfo *mi ) { if ( soundCache ) soundCache->PrintMemInfo( mi ); }
int idSoundSystemLocal::IsEAXAvailable() { return 0; }
const char *idSoundSystemLocal::GetDeviceName( int index ) { return index == 0 ? "Reconstructed software backend" : NULL; }
const char *idSoundSystemLocal::GetDefaultDeviceName() { return "Reconstructed software backend"; }
void idSoundSystemLocal::DisableAllSounds() { StopAllSounds( SOUNDWORLD_ANY ); ClearBuffer(); }
void idSoundSystemLocal::FreeSoundEmitter( int worldId, int handle, bool immediate ) {
	idSoundEmitterLocal *emitter = static_cast<idSoundEmitterLocal *>( EmitterForIndex( worldId, handle ) );
	if ( emitter == NULL ) return;
	if ( immediate ) {
		emitter->StopSound( SCHANNEL_ANY );
		emitter->removeStatus = REMOVE_STATUS_SAMPLEFINISHED;
	} else {
		emitter->removeStatus = REMOVE_STATUS_WAITSAMPLEFINISHED;
	}
}

void idSoundSystemLocal::PlayShaderDirectly( int worldId, const char *name, int channel ) {
	if ( name == NULL || name[0] == '\0' ) {
		if ( worldId >= 0 && worldId < SOUNDWORLD_MAX && localSound[worldId] ) localSound[worldId]->StopSound( channel );
		return;
	}
	const idSoundShader *shader = declManager->FindSound( name );
	if ( shader != NULL && worldId >= 0 && worldId < SOUNDWORLD_MAX && localSound[worldId] != NULL ) {
		localSound[worldId]->StartSound( shader, channel, 0.0f, 0 );
	}
}

int idSoundSystemLocal::SamplesToMilliseconds( int samples ) const { return (int)( ( (__int64)samples * 1000 ) / PRIMARYFREQ ); }
int idSoundSystemLocal::MillisecondsToSamples( int ms ) const { return (int)( ( (__int64)ms * PRIMARYFREQ ) / 1000 ); }
int idSoundSystemLocal::GetCurrent44kHzTime() const { return CurrentSoundTime; }
int idSoundSystemLocal::GetHardwareTime() const { return CurrentSoundTime; }

ALuint idSoundSystemLocal::AllocOpenALSource( idSoundChannel *chan, bool looping, bool stereo ) { return 0; }
void idSoundSystemLocal::FreeOpenALSource( ALuint handle ) {}

size_t idSoundSystemLocal::ListSoundSummary() { return soundCache ? soundCache->ListSoundSummary() : 0; }
bool idSoundSystemLocal::GetInsideLevelLoad() const { return soundCache != NULL && soundCache->insideLevelLoad; }
idSoundSample *idSoundSystemLocal::FindSample( const idStr &filename ) { return soundCache ? soundCache->FindSample( filename ) : NULL; }
bool idSoundSystemLocal::ValidateSoundShader( idSoundShader *shader ) { return shader != NULL; }
bool idSoundSystemLocal::HasCache() const { return soundCache != NULL; }
void idSoundSystemLocal::SetRenderWorld( idRenderWorld *renderWorld ) { rw = renderWorld; }
int idSoundSystemLocal::GetActiveSoundWorld() { return activeSoundWorld; }
int idSoundSystemLocal::GetNumAreas() { return areaReverbs.Num(); }
int idSoundSystemLocal::GetNumEmitters() const { return emitters.Num(); }
int idSoundSystemLocal::SetActiveSoundWorld( bool on ) { activeSoundWorld = on ? SOUNDWORLD_GAME : SOUNDWORLD_MENU; return activeSoundWorld; }
void idSoundSystemLocal::ListSoundDecoders( const idCmdArgs &args ) { common->Printf( "0 sound decoders\n" ); }
void idSoundSystemLocal::ActivateFilter( int filter ) {}
void idSoundSystemLocal::EndCinematic() { ClearBuffer(); }

void S_InitSoundSystem() {
	soundSystemLocal.Init();
}
