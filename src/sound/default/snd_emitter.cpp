...
void idSoundFade::Clear() {
	fadeStart44kHz = fadeEnd44kHz = 0;
	fadeStartVolume = fadeEndVolume = 0.0f;
}

float idSoundFade::FadeDbAt44kHz( int current44kHz ) {
	if ( current44kHz <= fadeStart44kHz ) return fadeStartVolume;
	if ( current44kHz >= fadeEnd44kHz || fadeEnd44kHz <= fadeStart44kHz ) return fadeEndVolume;
	const float fraction = (float)( current44kHz - fadeStart44kHz ) / (float)( fadeEnd44kHz - fadeStart44kHz );
	return fadeStartVolume + fraction * ( fadeEndVolume - fadeStartVolume );
}

idSoundChannel::idSoundChannel() { Clear(); }
idSoundChannel::~idSoundChannel() { Stop(); }
...
void idSoundChannel::Clear() {
	...
	lastShift44kHzTime = 0;
	lastFrequencyShift = 1.0f;
	elapsedShift44kHzTime = 0.0f;
}

void idSoundChannel::Stop() { ALStop(); Clear(); }
void idSoundChannel::ALStop() { if ( openalSource ) soundSystemLocal.FreeOpenALSource( openalSource ); openalSource = 0; }
int idSoundChannel::GatherChannelSamples( int sampleOffset44k, int sampleCount44k, float *dest ) const {
	if ( dest != NULL && sampleCount44k > 0 ) memset( dest, 0, sampleCount44k * sizeof( float ) );
	return 0;
}

idSoundEmitterLocal::idSoundEmitterLocal() { Clear(); }
idSoundEmitterLocal::~idSoundEmitterLocal() { StopSound( SCHANNEL_ANY ); }

void idSoundEmitterLocal::Clear() {
	worldId = SOUNDWORLD_NONE;
	index = 0;
	removeStatus = REMOVE_STATUS_ALIVE;
	origin.Zero();
	listenerId = 0;
	memset( &parms, 0, sizeof( parms ) );
	...
	realDistance = distance = 0.0f;
	numPortalChecks = 0;
	ampTime = 0;
	amplitude = 0.0f;
}

void idSoundEmitterLocal::ClearPortalArea() { lastValidPortalArea = -1; }
void idSoundEmitterLocal::OverrideParms( const soundShaderParms_t *base, const soundShaderParms_t *over, soundShaderParms_t *out ) {
	if ( out == NULL ) return;
	if ( base != NULL ) *out = *base; else memset( out, 0, sizeof( *out ) );
	if ( over != NULL ) *out = *over;
}
void idSoundEmitterLocal::CheckForCompletion( int current44kHzTime ) {
	playing = false;
	for ( int i = 0; i < SOUND_MAX_CHANNELS; ++i ) if ( channels[i].triggerState ) { playing = true; break; }
}
void idSoundEmitterLocal::Spatialize( idVec3 listenerPos, int listenerArea, idRenderWorld *rw ) {
	spatializedOrigin = origin;
	realDistance = distance = ( origin - listenerPos ).Length();
}
void idSoundEmitterLocal::UpdateEmitter( const idVec3 &newOrigin, const idVec3 &velocity, int newListenerId, const soundShaderParms_t *newParms ) {
	origin = newOrigin;
	listenerId = newListenerId;
	if ( newParms != NULL ) parms = *newParms;
}

int idSoundEmitterLocal::StartSound( const idSoundShader *shader, int channel, float newDiversity, int shaderFlags ) {
	if ( shader == NULL ) return 0;
	int slot = -1;
	for ( int i = 0; i < SOUND_MAX_CHANNELS; ++i ) {
		if ( channel != SCHANNEL_ANY && channels[i].triggerState && channels[i].triggerChannel == channel ) { slot = i; break; }
		if ( slot < 0 && !channels[i].triggerState ) slot = i;
	}
	if ( slot < 0 ) slot = 0;
	idSoundChannel &ch = channels[slot];
	ch.Clear();
	ch.triggerState = ch.triggered = true;
	ch.trigger44kHzTime = soundSystemLocal.GetCurrent44kHzTime();
	ch.triggerChannel = channel;
	ch.soundShader = shader;
	ch.diversity = newDiversity;
	shader->GetParms( &ch.parms );
	ch.parms.soundShaderFlags |= shaderFlags;
	playing = true;
	hasShakes = hasShakes || ch.parms.shakes > 0.0f;
	maxDistance = Max( maxDistance, ch.parms.maxDistance );
	return (int)( shader->GetTimeLength() * 1000.0f );
}

void idSoundEmitterLocal::ModifySound( int channel, const soundShaderParms_t *newParms ) {
	if ( newParms == NULL ) return;
	for ( int i = 0; i < SOUND_MAX_CHANNELS; ++i ) if ( channels[i].triggerState && ( channel == SCHANNEL_ANY || channels[i].triggerChannel == channel ) ) channels[i].parms = *newParms;
}
void idSoundEmitterLocal::StopSound( int channel ) {
	for ( int i = 0; i < SOUND_MAX_CHANNELS; ++i ) if ( channel == SCHANNEL_ANY || channels[i].triggerChannel == channel ) channels[i].Stop();
	CheckForCompletion( soundSystemLocal.GetCurrent44kHzTime() );
}
void idSoundEmitterLocal::FadeSound( int channel, float to, float over ) {
	const int now = soundSystemLocal.GetCurrent44kHzTime();
	const int end = now + (int)( over * PRIMARYFREQ );
	for ( int i = 0; i < SOUND_MAX_CHANNELS; ++i ) if ( channels[i].triggerState && ( channel == SCHANNEL_ANY || channels[i].triggerChannel == channel ) ) {
		channels[i].channelFade.fadeStart44kHz = now;
		channels[i].channelFade.fadeEnd44kHz = end;
		channels[i].channelFade.fadeStartVolume = channels[i].lastVolume;
		channels[i].channelFade.fadeEndVolume = to;
	}
}
bool idSoundEmitterLocal::CurrentlyPlaying( int channel ) const {
	for ( int i = 0; i < SOUND_MAX_CHANNELS; ++i ) if ( channels[i].triggerState && ( channel == SCHANNEL_ANY || channels[i].triggerChannel == channel ) ) return true;
	return false;
}
float idSoundEmitterLocal::CurrentAmplitude( int channelFlags, bool factorDistance ) { return amplitude; }
bool idSoundEmitterLocal::AttachedToWorld( int id ) const { return id == SOUNDWORLD_ANY || worldId == id; }
int idSoundEmitterLocal::Handle() const { return index; }
