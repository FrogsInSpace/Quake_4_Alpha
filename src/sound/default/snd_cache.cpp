...
idSoundSample::idSoundSample() :
	nChannels( 0 ), nSamplesPerSec( 0 ), objectSize( 0 ), objectMemSize( 0 ),
	nonCacheData( NULL ), openalBuffer( 0 ), hardwareBuffer( false ),
	wFormatTag( 0 ), wBitsPerSample( 0 ), nAvgBytesPerSec( 0 ), isOgg( false ) {
}

idSoundSample::~idSoundSample() { PurgeSoundSample(); }
const byte *idSoundSample::GetSampleData() const { return nonCacheData; }
int idSoundSample::GetNumChannels() const { return nChannels; }
int idSoundSample::GetNumSamples() const { return objectSize; }
int idSoundSample::GetSampleRate() const { return nSamplesPerSec; }
int idSoundSample::GetMemoryUsed() const { return objectMemSize; }
int idSoundSample::GetDurationMS() const { return nSamplesPerSec > 0 ? (int)( (__int64)objectSize * 1000 / nSamplesPerSec ) : 0; }
float idSoundSample::GetDuration() const { return GetDurationMS() * 0.001f; }
bool idSoundSample::IsOgg() const { return isOgg; }
int idSoundSample::LengthIn44kHzSamples() const { return nSamplesPerSec > 0 ? (int)( (__int64)objectSize * PRIMARYFREQ / nSamplesPerSec ) : 0; }

void idSoundSample::PurgeSoundSample() {
	if ( nonCacheData != NULL ) Mem_Free( nonCacheData );
	nonCacheData = NULL;
	objectSize = objectMemSize = 0;
	purged = true;
}

void idSoundSample::Load( int langIndex ) {
	purged = false;
	levelLoadReferenced = true;
}

void idSoundSample::Expand( bool force ) { isOgg = false; }
void idSoundSample::MakeDefault() { defaultSound = true; purged = false; nChannels = 1; nSamplesPerSec = PRIMARYFREQ; }
void idSoundSample::CheckForDownSample() {}
void idSoundSample::Reload( bool force ) { if ( force || purged ) Load(); }
unsigned int idSoundSample::GetNewTimeStamp() const { return timestamp; }

bool idSoundSample::FetchFromCache( int offset, const byte **output, int *position, int *size, bool allowIO ) {
	if ( offset < 0 || nonCacheData == NULL || offset >= objectMemSize ) return false;
	if ( output ) *output = nonCacheData + offset;
	if ( position ) *position = offset;
	if ( size ) *size = objectMemSize - offset;
	return true;
}

idSoundCache::idSoundCache() : insideLevelLoad( false ) {}
idSoundCache::~idSoundCache() { listCache.DeleteContents( true ); }

idSoundSample *idSoundCache::FindSample( const idStr &filename ) {
	for ( int i = 0; i < listCache.Num(); ++i ) if ( listCache[i] != NULL && !idStr::Icmp( listCache[i]->name, filename ) ) { listCache[i]->levelLoadReferenced = true; return listCache[i]; }
	idSoundSample *sample = new idSoundSample;
	sample->name = filename;
	sample->Load();
	listCache.Append( sample );
	return sample;
}

const idSoundSample *idSoundCache::GetObjectA( int index ) const { return index >= 0 && index < listCache.Num() ? listCache[index] : NULL; }
void idSoundCache::ReloadSounds( bool force ) { for ( int i = 0; i < listCache.Num(); ++i ) if ( listCache[i] ) listCache[i]->Reload( force ); }
void idSoundCache::PurgeSounds( bool force ) { for ( int i = 0; i < listCache.Num(); ++i ) if ( listCache[i] && ( force || !listCache[i]->levelLoadReferenced ) ) listCache[i]->PurgeSoundSample(); }
void idSoundCache::BeginLevelLoad( const char *mapName ) { insideLevelLoad = true; for ( int i = 0; i < listCache.Num(); ++i ) if ( listCache[i] ) listCache[i]->levelLoadReferenced = false; }
void idSoundCache::EndLevelLoad( const char *mapName ) { insideLevelLoad = false; PurgeSounds( false ); }
void idSoundCache::PrintMemInfo( MemInfo *mi ) {}
size_t idSoundCache::ListSoundSummary() { size_t total = 0; for ( int i = 0; i < listCache.Num(); ++i ) if ( listCache[i] ) total += listCache[i]->GetMemoryUsed(); return total; }

void idSoundSystemLocal::CleanCache() { if ( soundCache ) soundCache->PurgeSounds( false ); }
void *idSoundSystemLocal::AllocSoundSample( int size ) { return size > 0 ? Mem_Alloc( size, MA_SOUND ) : NULL; }
void idSoundSystemLocal::FreeSoundSample( const byte *address ) { if ( address ) Mem_Free( const_cast<byte *>( address ) ); }
int idSoundSystemLocal::GetCacheMemoryUsage() { return soundCache ? (int)soundCache->ListSoundSummary() : 0; }
