...
// Quake 4 keeps all sound worlds in a single sound-system object.  These
// declarations follow the retail PDB layouts; the old Doom 3 world-based
// declarations remain below as a reconstruction seed.
...
const int SOUND_DECODER_FREE_DELAY = 110250;
...
public:
	...
	void Stop();
	void ALStop();
	int GatherChannelSamples( int sampleOffset44k, int sampleCount44k, float *dest ) const;
	...
	int lastShift44kHzTime;
	float lastFrequencyShift;
	float elapsedShift44kHzTime;
};

class idSoundSample : public rvCommonSample {
public:
	idSoundSample();
	virtual ~idSoundSample();

	virtual const byte *GetSampleData() const;
	virtual int GetNumChannels() const;
	virtual int GetNumSamples() const;
	virtual int GetSampleRate() const;
	virtual int GetMemoryUsed() const;
	virtual int GetDurationMS() const;
	virtual float GetDuration() const;
	virtual void Load( int langIndex = -1 );
	virtual void PurgeSoundSample();
	virtual bool IsOgg() const;
	virtual void Expand( bool force );

	int LengthIn44kHzSamples() const;
	void MakeDefault();
	void CheckForDownSample();
	void Reload( bool force );
	unsigned int GetNewTimeStamp() const;
	bool FetchFromCache( int offset, const byte **output, int *position, int *size, bool allowIO );
...
public:
	idSoundCache();
	~idSoundCache();
	idSoundSample *FindSample( const idStr &filename );
	const idSoundSample *GetObjectA( int index ) const;
	void ReloadSounds( bool force );
	void PurgeSounds( bool force );
	void BeginLevelLoad( const char *mapName );
	void EndLevelLoad( const char *mapName );
	void PrintMemInfo( MemInfo *mi );
	size_t ListSoundSummary();
...
public:
	...
	virtual int StartSound( const idSoundShader *shader, int channel, float diversity, int shaderFlags );
	virtual void ModifySound( int channel, const soundShaderParms_t *parms );
	virtual void StopSound( int channel );
	virtual void FadeSound( int channel, float to, float over );
	virtual bool CurrentlyPlaying( int channel = SCHANNEL_ANY ) const;
	virtual float CurrentAmplitude( int channelFlags = -1, bool factorDistance = false );
	virtual bool AttachedToWorld( int id ) const;
	virtual int Handle() const;

	void Clear();
	void ClearPortalArea();
...
public:
	...
	virtual bool InitHW();
	virtual void InitVoiceComms();
	virtual bool ShutdownHW();
	virtual void ShutdownVoiceComms();
	virtual void Frame();
	virtual void ForegroundUpdate();
	...
	virtual int GetNumEmitters() const;
	virtual float CurrentShakeAmplitudeForPosition( int worldId, int time, const idVec3 &listenerPosition );
	virtual void PlaceListener( const idVec3 &origin, const idMat3 &axis, int listenerId, int gameTime, const idStr &areaName );
	virtual void ResetListener();
	virtual void FadeSoundClasses( int worldId, int soundClass, float to, float over );
	...
	virtual int GetHardwareTime() const;
	virtual int SetActiveSoundWorld( bool on );
	virtual int GetActiveSoundWorld();
	...
	virtual size_t ListSoundSummary();
	...
	virtual bool GetInsideLevelLoad() const;
	...
	idSoundEmitterLocal *AllocLocalSoundEmitter( int worldId, int index );
	void ClearAllSoundEmitters();
	void OffsetSoundTime( int offset44kHz );
	void AVIUpdate();
	int GetCacheMemoryUsage();
	void ListSoundDecoders( const idCmdArgs &args );
	void LoadReverbData( const char *mapName );
	void HandleVoiceComms();
	void ActivateFilter( int filter );

	static idCVar s_noSound;
	static idCVar s_maxSoundsPerShader;
	static idCVar s_volume;
	static idCVar s_useOpenAL;
	static idCVar s_deviceName;
	...
	idStr aviDemoPath;
	idStr aviDemoName;
	idSoundEmitterLocal *localSound[SOUNDWORLD_MAX];
	int activeSoundWorld;
	...
	dword *graph;
	idList<int> areaReverbs;
	ALCdevice *openalDevice;
	ALCcontext *openalContext;
	ALsizei openalSourceCount;
	idAudioSource openalSources[256];
	EAXSet alEAXSet;
	EAXGet alEAXGet;
...
public:
	virtual const char *GetSampleName( const idSoundShader *sound, int index ) const;
	virtual int GetSamplesPerSec( const idSoundShader *sound, int index ) const;
	virtual int GetNumChannels( const idSoundShader *sound, int index ) const;
	virtual int GetNumSamples( const idSoundShader *sound, int index ) const { return sound != NULL ? sound->GetNumSamples( index ) : 0; }
	virtual int GetMemorySize( const idSoundShader *sound, int index ) const;
	virtual const byte *GetNonCacheData( const idSoundShader *sound, int index ) const;
	virtual void LoadSampleData( idSoundShader *sound, int langIndex = -1 );
	virtual void ExpandSmallOggs( idSoundShader *sound, bool force );
	virtual const char *GetShakeData( idSoundShader *sound, int index );
	virtual void SetShakeData( idSoundShader *sound, int index, const char *ampData );
	virtual void Purge( idSoundShader *sound, bool freeBaseBlocks );
};

extern idSoundSystemLocal soundSystemLocal;

static_assert( sizeof( idSoundFade ) == 16, "Quake 4 PDB: idSoundFade" );
static_assert( sizeof( idSoundChannel ) == 152, "Quake 4 PDB: idSoundChannel" );
static_assert( sizeof( idSoundSample ) == 88, "Quake 4 PDB: idSoundSample" );
static_assert( sizeof( idSoundCache ) == 20, "Quake 4 PDB: idSoundCache" );
static_assert( sizeof( idSoundEmitterLocal ) == 1332, "Quake 4 PDB: idSoundEmitterLocal" );
static_assert( sizeof( idSoundSystemLocal ) == 105036, "Quake 4 PDB: idSoundSystemLocal" );
