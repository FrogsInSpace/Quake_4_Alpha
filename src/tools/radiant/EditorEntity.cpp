...
entity_t *Entity_PostParse(entity_t *ent, brush_t *pList) {
	...
	if (e->fixedsize || hasModel) {			// fixed size entity
		...
		if (hasModel) {
			...
			if ( dynamic_cast<idRenderModelLiquid*>( modelHandle ) ) {
				bo.Zero();
				bo.ExpandSelf( 12.0f );
			} else {
...
void Entity_UpdateSoundEmitter( entity_t *ent ) {
	...
	if ( g_pParentWnd->GetCamera()->GetSoundMode() 
		...
		if ( v[0] ) {
			...
			if ( !sound.waitfortrigger ) {	// waitfortrigger will not start playing immediately
				if ( !ent->soundEmitter ) {
					const int handle = soundSystem->AllocSoundEmitter( SOUNDWORLD_EDITOR );
					ent->soundEmitter = soundSystem->EmitterForIndex( SOUNDWORLD_EDITOR, handle );
				}
				playing = true;
	...
	if ( !playing && ent->soundEmitter ) {
		soundSystem->FreeSoundEmitter( SOUNDWORLD_EDITOR, ent->soundEmitter->Handle(), true );
		ent->soundEmitter = NULL;
	}
