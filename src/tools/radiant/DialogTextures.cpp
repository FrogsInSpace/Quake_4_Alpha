		...
		if (mode == SOUNDS) {
			idStr loadName;
 			if (!m_treeTextures.ItemHasChildren(item)) {
				...
				soundSystem->PlayShaderDirectly( SOUNDWORLD_EDITOR, actionName );
			} else {
				loadName = m_treeTextures.GetItemText(item);
			}

		} else {
			soundSystem->StopAllSounds( SOUNDWORLD_EDITOR );
		}
	}
}
	...
	*pResult = 0;
	...
	if (item) {
		...
		if ((dw == TEXTURES) || (dw == MATERIALS)) {
			...
			materialEdit->SetGui( const_cast<idMaterial *>( mat ), strParent );
			editGui = strParent;
...
void CDialogTextures::addParticles( bool rootItems ) {
	// Quake 4 uses BSE effects rather than Doom's .prt declarations.
}
...
void CDialogTextures::OnSize(UINT nType, int cx, int cy) 
{
	...
	// The Quake 4 inspector can resize this dialog while its child controls are
	// still being attached by MFC.  The hide-root option is not used by the Q4
	// material browser, so treat it as optional until it has a real HWND.
	if ( ::IsWindow(m_chkHideRoot.GetSafeHwnd()) ) {
