...
BOOL CInspectorDialog::OnInitDialog()
{
	...
	const BOOL consoleCreated = consoleWnd.Create(IDD_DIALOG_CONSOLE, this);
	const BOOL textureCreated = texWnd.Create(TEXTURE_WINDOW_CLASS, "", QE3_SPLITTER_STYLE, CRect(5, 5, 10, 10), this, 1299);
	const BOOL mediaCreated = mediaDlg.Create(IDD_DIALOG_TEXTURELIST, this);
	const BOOL entityCreated = entityDlg.Create(IDD_DIALOG_ENTITY, this);
	common->Printf(
		"Radiant inspector windows: console=%d/%d texture=%d/%d media=%d/%d entity=%d/%d\n",
		consoleCreated, ::IsWindow(consoleWnd.GetSafeHwnd()),
		textureCreated, ::IsWindow(texWnd.GetSafeHwnd()),
		mediaCreated, ::IsWindow(mediaDlg.GetSafeHwnd()),
		entityCreated, ::IsWindow(entityDlg.GetSafeHwnd()) );

	dockedTabs = GetCvarInt ( "radiant_InspectorDockedDialogs" , W_CONSOLE | W_TEXTURE | W_MEDIA );

	if ( consoleCreated && ::IsWindow(consoleWnd.GetSafeHwnd()) ) {
		AddDockedWindow ( &consoleWnd , W_CONSOLE , 1 , "Console", (dockedTabs & W_CONSOLE ) != 0 , InspectorsDockingCallback );
	}
	if ( textureCreated && ::IsWindow(texWnd.GetSafeHwnd()) ) {
		AddDockedWindow ( &texWnd, W_TEXTURE, 2, "Textures", (dockedTabs & W_TEXTURE ) != 0, InspectorsDockingCallback );
	}
	if ( mediaCreated && ::IsWindow(mediaDlg.GetSafeHwnd()) ) {
		AddDockedWindow ( &mediaDlg, W_MEDIA, 3, "Media", (dockedTabs & W_MEDIA ) != 0, InspectorsDockingCallback );
	}
	if ( entityCreated && ::IsWindow(entityDlg.GetSafeHwnd()) ) {
