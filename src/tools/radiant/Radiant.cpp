...
void RadiantShutdown( void ) {
	g_editorAlive = false;
	if ( g_pParentWnd != NULL && ::IsWindow( g_pParentWnd->GetSafeHwnd() ) ) {
		g_pParentWnd->DestroyWindow();
	}
	theApp.m_pMainWnd = NULL;
	if ( g_qeglobals.rw != NULL ) {
		soundSystem->SetRenderWorld( NULL );
		renderSystem->FreeRenderWorld( g_qeglobals.rw );
		g_qeglobals.rw = NULL;
	}
	g_DoomInstance = NULL;
...
void RadiantInit( void ) {
	...
	g_qeglobals.sw = SOUNDWORLD_EDITOR;
