...
BOOL CCamWnd::PreCreateWindow(CREATESTRUCT &cs) {
	...
	if (::GetClassInfo(hInstance, CAMERA_WINDOW_CLASS, &wc) == FALSE) {
		...
		if (AfxRegisterClass(&wc) == FALSE) {
			common->Warning("Radiant: failed to register %s (error %lu)", CAMERA_WINDOW_CLASS, GetLastError());
			return FALSE;
		}
	}
...
void CCamWnd::SetProjectionMatrix() {
	...
	//
	zNear	= cvarSystem->GetCVarFloat( "r_znear" );
...
void CCamWnd::Cam_Draw() {
	...
	soundSystem->PlaceListener( m_Camera.origin, axis, 0, Sys_Milliseconds(), "Undefined" );
...
void CCamWnd::BuildEntityRenderState( entity_t *ent, bool update) {
	...
	if ( v && *v ) {
		...
		refent.referenceSoundHandle = ent->soundEmitter ? ent->soundEmitter->Handle() : -1;
	...
	lightParms.referenceSoundHandle = ent->soundEmitter ? ent->soundEmitter->Handle() : -1;
	...
	if (brush->modelHandle > 0 ) {
		...
		if (model) {
			...
			for (int i = 0; i < model->NumSurfaces() ; i++) {
				...
				srfTriangles_t *tri2 = renderModelManager->CopyStaticTriSurf(tri);
...
int Brush_ToTris(brush_t *brush, idTriList *tris, idMatList *mats, bool models, bool bmodel) {
	...
	if ( brush->pPatch ) {
		...
		tri = renderModelManager->AllocStaticTriSurf( width * height, 6 * ( width - 1 ) * ( height - 1 ) );
	...
	for ( face_t *face = brush->brush_faces ; face; face = face->next ) {
		...
		tri = renderModelManager->AllocStaticTriSurf( w->GetNumPoints(), ( w->GetNumPoints() - 2 ) * 3 );
...
void Select_ToOBJ() {
	...
	if (dlgFile.DoModal() == IDOK) {
		...
		for( i = 0; i < tris.Num(); i++ ) {
			renderModelManager->FreeStaticTriSurf( tris[i] );
		}
		tris.Clear();
	}
}
