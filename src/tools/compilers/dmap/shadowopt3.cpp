...
static void ProjectUniqued( idVec3 projectionOrigin, idPlane projectionPlane ) {
	...
	renderSystem->LightProjectionMatrix( projectionOrigin, projectionPlane, mat );
...
srfTriangles_t *CreateLightShadow( optimizeGroup_t *shadowerGroups, const mapLight_t *light ) {;
	...
	renderModelManager->CleanupTriangles( occluders, false, true, false, false );
	...
	if ( dmapGlobals.shadowOptLevel == SO_MERGE_SURFACES ) {
		shadowTris = renderModelManager->CreateShadowVolume( occluders, light->def, SG_STATIC );
	} else {
		shadowTris = renderModelManager->CreateShadowVolume( occluders, light->def, SG_OFFLINE );
	}
	renderModelManager->FreeStaticTriSurf( occluders );
