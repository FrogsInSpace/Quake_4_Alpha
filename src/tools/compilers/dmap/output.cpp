...
srfTriangles_t	*ShareMapTriVerts( const mapTri_t *tris ) {
	...
	uTri = renderModelManager->AllocStaticTriSurf( count * 3, count * 3 );
...
static void CleanupUTriangles( srfTriangles_t *tri ) {
	...
	renderModelManager->SimpleCleanupTriangles( tri );
}
...
typedef struct interactionTris_s {
	...
	for ( group = area->groups ; group ; group = group->nextGroup ) {
		...
		renderModelManager->FreeStaticTriSurf( uTri );
...
void WriteOutputFile( void ) {
	...
	for ( i = 0 ; i < dmapGlobals.mapLights.Num() ; i++ ) {
		...
		renderModelManager->FreeStaticTriSurf( light->shadowTris );
