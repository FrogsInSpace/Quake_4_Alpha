...
static void CreateMapLight( const idMapEntity *mapEnt ) {
	...
	memset( &light->parms, 0, sizeof( light->parms ) );
	light->name[0] = '\0';
	light->shadowTris = NULL;
	light->def = renderModelManager->CreateLightDef();

	// parse parms exactly as the game do
	// use the game's epair parsing code so
	// we can use the same renderLight generation
	gameEdit->ParseSpawnArgsToRenderLight( &mapEnt->epairs, &light->parms );
	light->def->UpdateRenderLight( &light->parms, true );
	renderSystem->RenderLightFrustum( light->parms, light->frustum );
	light->globalLightOrigin = light->parms.origin + light->parms.axis * light->parms.lightCenter;
	if ( light->parms.parallel ) {
		idVec3 direction = light->parms.lightCenter;
		if ( !direction.Normalize() ) direction.Set( 0, 0, 1 );
		light->globalLightOrigin = light->parms.origin + direction * 100000.0f;
	}
	light->lightShader = light->parms.shader;
	if ( !light->lightShader ) {
		light->lightShader = declManager->FindMaterial( light->parms.pointLight ?
			"lights/defaultPointLight" : "lights/defaultProjectedLight" );
	}
	idWinding *frustumWindings[6] = { NULL, NULL, NULL, NULL, NULL, NULL };
	srfTriangles_t *frustumTris = renderModelManager->PolytopeSurface( 6, light->frustum, frustumWindings );
	if ( frustumTris ) {
		light->frustumBounds = frustumTris->bounds;
		renderModelManager->FreeStaticTriSurf( frustumTris );
	} else {
		light->frustumBounds = idBounds( light->parms.origin ).Expand( light->parms.lightRadius.Length() );
	}
	for ( int windingNum = 0; windingNum < 6; windingNum++ ) delete frustumWindings[windingNum];
...
void FreeDMapFile( void ) {
	...
	for ( i = 0; i < dmapGlobals.mapLights.Num(); i++ ) {
		renderModelManager->FreeLightDef( dmapGlobals.mapLights[i]->def );
	}
