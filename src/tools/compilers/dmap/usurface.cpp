...
static void ClipTriByLight( const mapLight_t *light, const mapTri_t *tri, 
	...
	*out = NULL;
	...
	for ( i = 0 ; i < 6 ; i++ ) {
		oldInside = inside;
		if ( oldInside ) {
			oldInside->Split( light->frustum[i], 0, &outside[i], &inside );
			delete oldInside;
		}
		else {
			outside[i] = NULL;
...
static void BuildLightShadows( uEntity_t *e, mapLight_t *light ) {
	...
	// shadow volume
	shadowerGroups = NULL;
	lightOrigin = light->globalLightOrigin;
	...
	if ( !light->parms.noShadows
		...
		for ( i = 0 ; i < e->numAreas ; i++ ) {
			for ( group = e->areas[i].groups ; group ; group = group->nextGroup ) {
				...
				// skip it
				if ( !group->bounds.IntersectsBounds( light->frustumBounds ) ) {
					continue;
				}
...
static void CarveGroupsByLight( uEntity_t *e, mapLight_t *light ) {
	...
	for ( i = 0 ; i < e->numAreas ; i++ ) {
		...
		for ( group = area->groups ; group ; group = nextGroup ) {
			...
			if ( ( light->lightShader->IsFogLight() && !group->material->ReceivesFog() )
				|| ( !light->lightShader->IsFogLight() && !group->material->ReceivesLighting() ) 
				|| !group->bounds.IntersectsBounds( light->frustumBounds ) ) {
			...
			if ( !light->lightShader->LightEffectsBackSides() &&
				!group->material->ReceivesLightingOnBackSides() &&
				dmapGlobals.mapPlanes[ group->planeNum ].Distance( light->parms.origin ) <= 0  ) {
