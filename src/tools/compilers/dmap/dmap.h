...
// Quake 4 moved the offline shadow optimizer behind the Tools API.  The
// compiler still uses Doom's result shape internally before translating it
// to SOOData_s for the renderer callback.
typedef struct optimizedShadow_s {
	idVec3 *verts;
	int numVerts;
	glIndex_t *indexes;
