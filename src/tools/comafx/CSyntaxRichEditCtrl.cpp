...
void CSyntaxRichEditCtrl::GetText( idStr &text, int startCharIndex, int endCharIndex ) const {
	...
	m_TextDoc->Range( startCharIndex, endCharIndex, &range );
	range->get_Text( &bstr );
	const int convertedLength = WideCharToMultiByte( CP_ACP, 0, bstr, -1, NULL, 0, NULL, NULL );
	if ( convertedLength > 0 ) {
		char *converted = new char[convertedLength];
		WideCharToMultiByte( CP_ACP, 0, bstr, -1, converted, convertedLength, NULL, NULL );
		text = converted;
		delete[] converted;
	} else {
		text.Clear();
